/*
 * $Id: kuwhat.c,v 1.16 2010/03/24 10:35:52 barrand Exp $
 *
 * $Log: kuwhat.c,v $
...
#pragma linkage(LNRDPAS,OS)
    ...
    if( kc_window.use_getline ) {
      ...
      if( (buf = kgl_getline( prompt )) == NULL )
        eof = 1;
      else
        strcpy( &line[len], buf );
      kgl_config( "noecho", 0 );
...
void ku_action_data( /*GB*/
 void* aData
)
{
  kjmpaddr.action_data = aData;
}
void ku_prompt( /*GB*/
 char*(*aFunction)(char*,void*)
,void* aData
)
{
...
void ku_xvvalue( /*GB*/
 int(*aFunction)(const char*,double*,void*)
,void* aData
)
{
