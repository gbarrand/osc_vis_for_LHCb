...
public:
  ...
  enum SoXtFullViewerPopupMenuItem {
  ...
  enum SoXtViewerPopupMenuRadioGroup {
    STILL_GROUP = 1,
      MOVE_GROUP,
      TRANSPARENCY_GROUP,
      BUFFER_GROUP,
  ...
  class SoXtPopupMenu * setupStandardPopupMenu(void);
  void prepareMenu(class SoXtPopupMenu * menu);
  void menuSelection(int menuitemid);
  static void menuSelectionCallback(int menuitemid, void * userdata);
  SbString popupmenutitle;

  void setDrawStyleMenuActivation(SoXtViewer::DrawType type,
...
protected:
  SoGuiFullViewerP(SoXtFullViewer * publ);
  SoXtFullViewer * pub;
};
