...
/*!
  ...
  SoXt::setFatalErrorHandler().

  Returns previous fatal error handler callback, or \c NULL if none.

  \sa SoXt::setFatalErrorHandler()
 */
SoXt::FatalErrorCB *
SoAny::setFatalErrorHandler(SoXt::FatalErrorCB * cb, void * userdataarg)
{
  SoXt::FatalErrorCB * oldcb = this->fatalcb;
  this->fatalcb = cb;
 ...
 */
SbBool
SoAny::invokeFatalErrorHandler(SbString errmsg, SoXt::FatalErrors errcode)
{
  ...
  if (this->fatalcb == NULL) {
    SoXt::createSimpleErrorDialog(NULL,
...
SoAny::getenv(const char * envname)
{
  /* Important note: this code is dulplicated in Coin, Coin-1,
     SoXt/.../common/SoAny.cpp.in (and possibly stable forks of the
