 ...
 *  A WORD OF ADVICE
 *
 *  It is fruitless to modify the contents of the SoXtRenderArea.cpp file
...
/*!
  \class SoXtRenderArea Inventor/Xt/SoXtRenderArea.h
  \brief The SoXtRenderArea class adds scenegraph handling and event management.
  \ingroup components viewers

  The SoXtRenderArea class is a component that adds scenegraph
  management and input device event handling to the SoXtGLWidget
  component.
  ...
  Native toolkit events are caught by SoXtRenderArea components,
  ...
  SoXtRenderArea is the first non-abstract component in it's
  inheritance hierarchy that you can use directly from client
  application code to set up a scenegraph viewer canvas.

  For an SoXtRenderArea component to properly display your
  ...
  SoXtRenderArea with a scenegraph:
...
#include <Inventor/Xt/common/gl.h> // glDrawBuffer()
...
SOXT_OBJECT_SOURCE(SoXtRenderArea);
...
public:
  ...
  SoXtRenderAreaP(class SoXtRenderArea * pub);
  ~SoXtRenderAreaP(void);
  ...
  const SoEvent * getSoEvent(XAnyEvent * event);

  SoXtRenderAreaEventCB * appeventhandler;
  void * appeventhandlerdata;

private:
  SoXtRenderArea * pub; // public interface class
  SbString currentinput; // For the OpenGL info-window hack.
};

const int SoXtRenderAreaP::GL_DEFAULT_MODE = (SO_GL_RGB |
                                                 SO_GL_ZBUFFER |
                                                 SO_GL_DOUBLE );

#if SOXT_DEBUG && defined(__COIN__)
...
#endif // SOXT_DEBUG && __COIN__

// Note: assumes a valid current OpenGL context.
void
SoXtRenderAreaP::showOpenGLDriverInformation(void)
{
#if DEBUGGING_EGGS
  ...
  SoXt::createSimpleErrorDialog(NULL, "OpenGL driver information",
                                    info.getString());
#endif // DEBUGGING_EGGS
}

void
SoXtRenderAreaP::showInventorInformation(void)
{
#if DEBUGGING_EGGS
  SbString info;
  info.sprintf("%s\n", SoDB::getVersion());
  ...
  SoXt::createSimpleErrorDialog(NULL, "Inventor implementation info",
                                    info.getString());
...
void
SoXtRenderAreaP::showToolkitInformation(void)
{
#if DEBUGGING_EGGS
  SbString info = "SoXt version "; info += SOXT_VERSION; info += "\n";
#if SOXT_MAKE_DLL
  ...
  // FIXME: information about DLL path(s) (both for the SoXt and
  ...
  SoXt::createSimpleErrorDialog(NULL, "SoXt implementation info",
                                    info.getString());
#endif // DEBUGGING_EGGS
}

void
SoXtRenderAreaP::dumpScenegraph(void)
{
  ...
  if (!ok) {
    SoDebugError::post("SoXtRenderAreaP::dumpScenegraph",
                       "couldn't open file '%s'", filename.getString());
    return;
  }
  SoWriteAction wa(&out);
  wa.apply(this->normalManager->getSceneGraph());
  SoDebugError::postInfo("SoXtRenderAreaP::dumpScenegraph",
                         "dumped scenegraph to '%s'", filename.getString());
#endif // DEBUGGING_EGGS
}

void
SoXtRenderAreaP::dumpCameras(void)
{
#ifdef DEBUGGING_EGGS
  ...
  SoDebugError::postInfo("SoXtRenderAreaP::dumpCameras",
...
int
SoXtRenderAreaP::checkMagicSequences(const char c)
{
#if DEBUGGING_EGGS
  this->currentinput += c;

  if (0) { // handy for debugging keyboard handling
    SoDebugError::postInfo("SoXtRenderAreaP::checkMagicSequences",
  ...
  static const int id[] = {
    SoXtRenderAreaP::OPENGL,
    SoXtRenderAreaP::INVENTOR,
    SoXtRenderAreaP::TOOLKIT,
    SoXtRenderAreaP::DUMPSCENEGRAPH,
    SoXtRenderAreaP::DUMPCAMERAS
  };
  ...
  return SoXtRenderAreaP::NONE;
}

// This method sets the window size data in all the connected device
// classes.
void
SoXtRenderAreaP::setDevicesWindowSize(const SbVec2s size)
{
  if (!this->devicelist) return;
  const int num = this->devicelist->getLength();
  for (int i = 0; i < num; i++)
    ((SoXtDevice *)(*this->devicelist)[i])->setWindowSize(size);
}

// *************************************************************************

void
SoXtRenderAreaP::renderCB(void * closure, SoSceneManager * manager)
{
  SoXtRenderArea * thisptr = (SoXtRenderArea *) closure;
...
void
SoXtRenderAreaP::selection_redraw_cb(void * closure, SoSelection * sel)
{
  SoXtRenderArea * ra = (SoXtRenderArea *) closure;
...
SoXtRenderAreaP::SoXtRenderAreaP(SoXtRenderArea * api)
{
  PUBLIC(this) = api;
...
SoXtRenderAreaP::~SoXtRenderAreaP()
{
...
void
SoXtRenderAreaP::constructor(SbBool mouseInput,
                                SbBool keyboardInput,
                                SbBool build)
{
  this->normalManager->setRenderCallback(SoXtRenderAreaP::renderCB, PUBLIC(this));
  this->normalManager->activate();
  this->overlayManager->setRenderCallback(SoXtRenderAreaP::renderCB, PUBLIC(this));
  ...
  // SoXtRenderArea instances with overlays? They will all use
  ...
  if (mouseInput) {
    this->devices.mouse = new SoXtMouse;
    PUBLIC(this)->registerDevice(this->devices.mouse);
  }

  if (keyboardInput) {
    this->devices.keyboard = new SoXtKeyboard;
    PUBLIC(this)->registerDevice(this->devices.keyboard);
  }

  if (! build) return;
  PUBLIC(this)->setClassName("SoXtRenderArea");
...
SbBool
SoXtRenderAreaP::invokeAppCB(XAnyEvent * event)
{
...
const SoEvent *
SoXtRenderAreaP::getSoEvent(XAnyEvent * event)
{
  if (!this->devicelist)
    return (SoEvent *) NULL;
  ...
  for (int i = 0; (i < num) && (soevent == NULL); i++)
    soevent = ((SoXtDevice *)(*this->devicelist)[i])->translateEvent(event);
...
/*!
  Public constructor.
*/
SoXtRenderArea::SoXtRenderArea(Widget parent,
                                     const char * name,
                                     SbBool embed,
                                     SbBool mouseInput,
                                     SbBool keyboardInput)
  : inherited(parent, name, embed, SoXtRenderAreaP::GL_DEFAULT_MODE, FALSE)
{
  PRIVATE(this) = new SoXtRenderAreaP(this);
...
SoXtRenderArea::~SoXtRenderArea()
{
  ...
  for (int i = PRIVATE(this)->devicelist->getLength() - 1; i >= 0; i--) {
    SoXtDevice * device = (SoXtDevice *) ((*PRIVATE(this)->devicelist)[i]);
...
SoXtRenderArea::registerDevice(SoXtDevice * device)
{
  ...
  if (idx != -1) {
#if SOXT_DEBUG
    SoDebugError::postWarning("SoXtRenderArea::registerDevice",
  ...
  if (w != NULL) {
#ifdef __COIN_SOXT__
    device->enable(w, (SoXtEventHandler *) &SoXtGLWidget::eventHandler, (void *)this);
#else
    device->enable(w, &SoXtGLWidgetP::eventHandler, (void *)this);
#endif
...
SoXtRenderArea::unregisterDevice(SoXtDevice * device)
{
  ...
  if (idx == -1) {
#if SOXT_DEBUG
    SoDebugError::post("SoXtRenderArea::unregisterDevice",
...
void
SoXtRenderArea::afterRealizeHook(void)
{
  inherited::afterRealizeHook();

#ifdef HAVE_JOYSTICK_LINUX
  if (SoXtLinuxJoystick::exists())
    this->registerDevice(new SoXtLinuxJoystick);
#endif // HAVE_JOYSTICK_LINUX

  if (SoXtSpaceball::exists())
    this->registerDevice(new SoXtSpaceball);
}
...
*/
void
SoXtRenderArea::setSceneGraph(SoNode * scene)
{
...
/*!
  ...
  \sa SoXtRenderArea::getSceneManager()
*/
SoNode *
SoXtRenderArea::getSceneGraph(void)
{
...
*/
void
SoXtRenderArea::setOverlaySceneGraph(SoNode * scene)
{
...
*/
SoNode *
SoXtRenderArea::getOverlaySceneGraph(void)
{
...
*/
void
SoXtRenderArea::setBackgroundColor(const SbColor & color)
{
...
*/
const SbColor &
SoXtRenderArea::getBackgroundColor(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
void
SoXtRenderArea::setBackgroundIndex(int idx)
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
int
SoXtRenderArea::getBackgroundIndex(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
void
SoXtRenderArea::setOverlayBackgroundIndex(int idx)
{
...
*/
int
SoXtRenderArea::getOverlayBackgroundIndex(void) const
{
  assert(PRIVATE(this)->overlayManager != NULL);
...
*/
void
SoXtRenderArea::setColorMap(int start, int num, const SbColor * colors)
{
...
*/
void
SoXtRenderArea::setOverlayColorMap(int start, int num,
...
*/
void
SoXtRenderArea::setViewportRegion(const SbViewportRegion & region)
{
  if (region.getWindowSize()[0] == -1) return;

#if SOXT_DEBUG && 0 // debug
  SoDebugError::postInfo("SoXtRenderArea::setViewportRegion",
                         "size=<%d, %d>",
...
SoXtRenderArea::getViewportRegion(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
SoXtRenderArea::getTransparencyType(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
void
SoXtRenderArea::getAntialiasing(SbBool & smoothing, int & numPasses) const
{
...
*/
void
SoXtRenderArea::setClearBeforeRender(SbBool enable, SbBool zbEnable)
{
...
*/
SbBool
SoXtRenderArea::isClearBeforeRender(void) const
{
...
*/
SbBool
SoXtRenderArea::isClearZBufferBeforeRender(void) const
{
...
*/
void
SoXtRenderArea::setClearBeforeOverlayRender(SbBool enable)
{
...
*/
SbBool
SoXtRenderArea::isClearBeforeOverlayRender(void) const
{
...
*/
void
SoXtRenderArea::setAutoRedraw(SbBool enable)
{
  if (enable) {
    PRIVATE(this)->normalManager->setRenderCallback(SoXtRenderAreaP::renderCB, this);
    PRIVATE(this)->overlayManager->setRenderCallback(SoXtRenderAreaP::renderCB, this);
  }
  else {
...
*/
SbBool
SoXtRenderArea::isAutoRedraw(void) const
{
...
*/
void
SoXtRenderArea::setRedrawPriority(uint32_t priority)
{
...
*/
uint32_t
SoXtRenderArea::getRedrawPriority(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
uint32_t
SoXtRenderArea::getDefaultRedrawPriority(void)
{
...
*/
void
SoXtRenderArea::renderOverlay(void)
{
  this->redrawOverlay();
}
...
*/
void
SoXtRenderArea::scheduleRedraw(void)
{
#if SOXT_DEBUG && RENDERAREA_DEBUG_REDRAWS // debug
  SoDebugError::postInfo("SoXtRenderArea::scheduleRedraw",
                         "invoked");
#endif // debug
...
*/
void
SoXtRenderArea::scheduleOverlayRedraw(void)
{
  assert(PRIVATE(this)->overlayManager != NULL);
  // In case autoRedraw is OFF. The callback pointer will be reset to
  // NULL in renderCB() if autoRedraw is OFF.
...
#ifndef DOXYGEN_SKIP_THIS
void
SoXtRenderAreaP::replaceSoSelectionMonitor(SoSelection * newsel,
                                              SoSelection * oldsel) const
{
  if (newsel) { newsel->ref(); }

  if (oldsel) {
    oldsel->removeChangeCallback(SoXtRenderAreaP::selection_redraw_cb,
                                 PUBLIC(this));
    oldsel->unref();
  }

  if (newsel) {
    newsel->addChangeCallback(SoXtRenderAreaP::selection_redraw_cb,
                              PUBLIC(this));
  }
}
#endif // DOXYGEN_SKIP_THIS
...
*/
void
SoXtRenderArea::redrawOnSelectionChange(SoSelection * selection)
{
...
/*!
  ...
  \sa SoXtRenderArea::redrawOnSelectionChange()
*/
void
SoXtRenderArea::redrawOverlayOnSelectionChange(SoSelection * selection)
{
...
*/
void
SoXtRenderArea::setEventCallback(SoXtRenderAreaEventCB * func,
                                    void * user)
{
...
*/
void
SoXtRenderArea::setSceneManager(SoSceneManager * manager)
{
  assert(PRIVATE(this)->normalManager != NULL);
...
/*!
  ...
  SoXtRenderArea subclass SoXtViewer and further subclasses. The
  ...
  SoXtRenderArea, then use SoSceneManager::getSceneGraph() to get
  the root node Coin uses for rendering.
*/
SoSceneManager *
SoXtRenderArea::getSceneManager(void) const
{
...
*/
void
SoXtRenderArea::setOverlaySceneManager(SoSceneManager * manager)
{
...
*/
SoSceneManager *
SoXtRenderArea::getOverlaySceneManager(void) const
{
...
*/
void
SoXtRenderArea::setGLRenderAction(SoGLRenderAction * action)
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
SoGLRenderAction *
SoXtRenderArea::getGLRenderAction(void) const
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
void
SoXtRenderArea::setOverlayGLRenderAction(SoGLRenderAction * action)
{
  assert(PRIVATE(this)->overlayManager != NULL);
...
*/
SoGLRenderAction *
SoXtRenderArea::getOverlayGLRenderAction(void) const
{
  assert(PRIVATE(this)->overlayManager != NULL);
...
*/
void
SoXtRenderArea::redraw(void)
{
#if SOXT_DEBUG && RENDERAREA_DEBUG_REDRAWS // debug
  SoDebugError::postInfo("SoXtRenderArea::redraw",
...
#ifdef __APPLE__
  //G.Barrand : Darwin : XQuartz 2.3.2 :
  //            The upper induces a GL_INVALID_OPERATION.
 {GLenum glerr = glGetError();
  if(glerr == GL_INVALID_OPERATION) {
    //printf("debug : SoXtRenderArea::redraw : GL error\n");    
  }}
#endif
...
#if SOXT_DEBUG && RENDERAREA_DEBUG_REDRAWS // debug
  SoDebugError::postInfo("SoXtRenderArea::render", "done");
#endif // debug
}
...
*/
void
SoXtRenderArea::actualRedraw(void)
{
  assert(PRIVATE(this)->normalManager != NULL);
...
*/
void
SoXtRenderArea::redrawOverlay(void)
{
...
*/
void
SoXtRenderArea::actualOverlayRedraw(void)
{
  assert(PRIVATE(this)->overlayManager != NULL);
  if (! this->isVisible()) return;
...
*/
void
SoXtRenderArea::initGraphic(void)
{
...
*/
void
SoXtRenderArea::initOverlayGraphic(void)
{
...
// doc in super
void
SoXtRenderArea::sizeChanged(const SbVec2s & size)
{
#if SOXT_DEBUG && 0
  SoDebugError::postInfo("SoXtRenderArea::sizeChanged",
  ...
  if (!forcenoresizeworkaround || (atoi(forcenoresizeworkaround) == 0)) {
    ...
    if (this->getTypeId() == SoXtRenderArea::getClassTypeId()) { 
...
// Documented in superclass.
void
SoXtRenderArea::widgetChanged(Widget widget)
{
  ...
  if (PRIVATE(this)->devicelist != NULL) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      device->enable(this->getGLWidget(),
                     (SoXtEventHandler *) &SoXtGLWidget::eventHandler,
                     (void *) this);
#else
      device->enable(this->getGLWidget(),
                     &SoXtGLWidgetP::eventHandler, (void *) this);
#endif
    }
  }
...
// Documented in superclass.
const char *
SoXtRenderArea::getDefaultWidgetName(void) const
{
  static const char defaultWidgetName[] = "SoXtWidgetName";
  return defaultWidgetName;
}

// Documented in superclass.
const char *
SoXtRenderArea::getDefaultTitle(void) const
{
  static const char defaultTitle[] = "Xt RenderArea";
  return defaultTitle;
}

// Documented in superclass.
const char *
SoXtRenderArea::getDefaultIconTitle(void) const
{
  static const char defaultIconTitle[] = "Xt RenderArea";
...
/*!
  ...
  in the SoXtRenderArea::processEvent() method.  If this succeeds,
...
*/
SbBool
SoXtRenderArea::processSoEvent(const SoEvent * const event)
{
...
/*!
  Overrides SoXtGLWidget::processEvent() to attempt to convert
  toolkit-native events to Coin-generic events.  If this succeeds, the
  generic SoEvent is forwarded to SoXtRenderArea::processSoEvent().
 */
void
SoXtRenderArea::processEvent(XAnyEvent * event)
{
  ...
  if (soevent != NULL) {
    ...
    if (soevent->isOfType(SoKeyboardEvent::getClassTypeId())) {
      ...
      if (ke->getState() == SoButtonEvent::UP) {
        ...
        case SoXtRenderAreaP::NONE:
          break;
        case SoXtRenderAreaP::OPENGL:  // "glinfo"
          this->glLockNormal();
          PRIVATE(this)->showOpenGLDriverInformation();
          this->glUnlockNormal();
          break;
        case SoXtRenderAreaP::INVENTOR:  // "ivinfo"
          PRIVATE(this)->showInventorInformation();
          break;
        case SoXtRenderAreaP::TOOLKIT:  // "soinfo"
          PRIVATE(this)->showToolkitInformation();
          break;
        case SoXtRenderAreaP::DUMPSCENEGRAPH:  // "dumpiv"
          PRIVATE(this)->dumpScenegraph();
          break;
        case SoXtRenderAreaP::DUMPCAMERAS:  // "cameras"
...
// doc from parent
SbBool 
SoXtRenderArea::glScheduleRedraw(void)
{
  this->scheduleRedraw();
...
/*!
  ...
  SoXtRenderArea-based component and returns the result value from the
  event handler.  This is a synchronous operation.
*/
SbBool
SoXtRenderArea::sendSoEvent(const SoEvent * event)
{
