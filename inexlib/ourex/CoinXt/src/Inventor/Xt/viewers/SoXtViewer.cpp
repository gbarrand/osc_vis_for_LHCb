...
// NOTE: The SoXtViewer.cpp sourcecode file is completely
// autogenerated from "templatized" source code.

// *************************************************************************

/*!
  \class SoXtViewer Inventor/Xt/viewers/SoXtViewer.h
  \brief The SoXtViewer class is the top level base viewer class.
  \ingroup components viewers

  This is an abstract class, which adds the following features to it's
  SoXtRenderArea superclass: convenient methods for camera
  ...
  SoXtViewer by reading mouse and keyboard events and interpreting
  ...
  See e.g. the source code for SoXtExaminerViewer::processSoEvent()
  for the details.



  The SoXtViewer class automatically adds a headlight to the scene,
  ...
  behavior can be turned off with SoXtViewer::setHeadlight()).


  SoXtViewer-derived viewers all inherit the following keyboard
  controls from this class (but only when the viewer is in "examine
  mode", ie SoXtViewer::isViewing() returns \c TRUE):
...
/*!
  \enum SoXtViewer::AutoClippingStrategy
...
/*!
  \var SoXtViewer::AutoClippingStrategy SoXtViewer::CONSTANT_NEAR_PLANE

  Constant near plane auto clipping strategy. Explained in detail in
  the documentation for the SoXtViewer::setAutoClippingStrategy()
  method.
*/

/*!
  \var SoXtViewer::AutoClippingStrategy SoXtViewer::VARIABLE_NEAR_PLANE
...
// SoXtViewerP::recordFPS() function below for more information.
static int COIN_SHOW_FPS_COUNTER = UNINITIALIZED_ENVVAR;

// *************************************************************************

// The private data for the SoXtViewer.
class SoXtViewerP {
public:
  SoXtViewerP(SoXtViewer * publ);
  ~SoXtViewerP(void);
  ...
  // Drawstyles
  SoXtViewer::DrawStyle drawstyles[2];
  ...
  float stereooffset;
  SoXtViewer::StereoType stereotype;
  ...
  SoXtViewer::StereoType stereostenciltype;
  ...
  void setStereoEye(SoCamera * camera,
                    const SoXtViewerP::Eye eye,
                    SoXtViewerP::StereoData & storage) const;
  ...
  SoXtViewer::BufferType buffertype;
  ...
  // Misc
  static void interactivestartCB(void *, SoXtViewer * thisp);
  static void interactiveendCB(void *, SoXtViewer * thisp);
  ...
  SoXtViewer::AutoClippingStrategy autoclipstrategy;
  float autoclipvalue;
  SoXtAutoClippingCB * autoclipcb;
...
SoXtViewerP::SoXtViewerP(SoXtViewer * publ)
{
  PUBLIC(this) = publ;
...
SoSeparator *
SoXtViewerP::createSuperScene(void)
{
  static const char * superSceneGraph[] =
  {
    ...
    "  DEF soxt->headlight DirectionalLight {",
    "    direction 1 -1 -10",
    "  }",

    "  DEF soxt->drawstyleroot Switch {",
    "    whichChild -1",
    "    DEF soxt->lightmodel LightModel {",
    "      model BASE_COLOR",
    "    }",
    "    DEF soxt->drawstyle DrawStyle {",
    "      pointSize ~",
    "      lineWidth ~",
    "      linePattern ~",
    "    }",
    "    DEF soxt->complexity Complexity {",
    "      textureQuality 0.0",
    "      value 0.1",
    "    }",
    "  }",
    "  DEF soxt->hiddenlineroot Switch {",
    "    whichChild -1",
    "    DEF soxt->basecolor BaseColor { }",
    "    DEF soxt->materialbinding MaterialBinding {",
    "      value OVERALL",
    "    }",
    "    DEF soxt->polygonoffsetparent Switch {",
    "      whichChild -1",
#ifdef HAVE_SOPOLYGONOFFSET
    "      DEF soxt->polygonoffset PolygonOffset { }",
#endif // HAVE_SOPOLYGONOFFSET
    "    }",
    "  }",
    "  DEF soxt->userscenegraphroot Separator {",
    "  }",
    "}",
    NULL
  };
  ...
  if (!ok) {
    ...
    SoDebugError::post("SoXtViewerP::createSuperScene",
...
#define LOCATE_NODE(member, type, name) \
  ...
  LOCATE_NODE(this->headlight, SoDirectionalLight, "soxt->headlight");
  LOCATE_NODE(this->drawstyleroot, SoSwitch, "soxt->drawstyleroot");
  LOCATE_NODE(this->hiddenlineroot, SoSwitch, "soxt->hiddenlineroot");
  LOCATE_NODE(this->polygonoffsetparent, SoSwitch,
	      "soxt->polygonoffsetparent");
  LOCATE_NODE(this->usersceneroot, SoSeparator, "soxt->userscenegraphroot");

  LOCATE_NODE(this->sobasecolor, SoBaseColor, "soxt->basecolor");
  LOCATE_NODE(this->socomplexity, SoComplexity, "soxt->complexity");
  LOCATE_NODE(this->sodrawstyle, SoDrawStyle, "soxt->drawstyle");
  LOCATE_NODE(this->solightmodel, SoLightModel, "soxt->lightmodel");
  LOCATE_NODE(this->somaterialbinding, SoMaterialBinding, "soxt->materialbinding");
...
#ifdef HAVE_SOPOLYGONOFFSET
  LOCATE_NODE(this->sopolygonoffset, SoPolygonOffset, "soxt->polygonoffset");
...
// in the scene graph.
void
SoXtViewerP::getCameraCoordinateSystem(SoCamera * cameraarg,
                                          SoNode * root,
                                          SbMatrix & matrix,
                                          SbMatrix & inverse)
{
...
void
SoXtViewerP::convertOrtho2Perspective(const SoOrthographicCamera * in,
                                         SoPerspectiveCamera * out)
{
  ...
  SoDebugError::postInfo("SoXtViewerP::convertOrtho2Perspective",
...
void
SoXtViewerP::convertPerspective2Ortho(const SoPerspectiveCamera * in,
                                         SoOrthographicCamera * out)
{
  out->aspectRatio.setValue(in->aspectRatio.getValue());
  out->focalDistance.setValue(in->focalDistance.getValue());
...
void
SoXtViewerP::reallyRedraw(const SbBool clearcol, const SbBool clearz)
{
  ...
  SoXtViewer::DrawStyle style = this->currentDrawStyle();
  switch (style) {
...
SbBool
SoXtViewerP::drawInteractiveAsStill(void) const
{
  SbBool moveasstill = this->drawstyles[SoXtViewer::INTERACTIVE] == SoXtViewer::VIEW_SAME_AS_STILL;
  if (! moveasstill)
    moveasstill = this->drawstyles[SoXtViewer::INTERACTIVE] == this->drawstyles[SoXtViewer::STILL];
  if (! moveasstill)
    moveasstill =
      this->drawstyles[SoXtViewer::INTERACTIVE] == SoXtViewer::VIEW_NO_TEXTURE &&
      this->drawstyles[SoXtViewer::STILL] != SoXtViewer::VIEW_AS_IS;
  return moveasstill;
}

// Returns the current drawing style.
SoXtViewer::DrawStyle
SoXtViewerP::currentDrawStyle(void) const
{
  SbBool interactivemode = PUBLIC(this)->getInteractiveCount() > 0 ? TRUE : FALSE;

  if (!interactivemode || this->drawInteractiveAsStill())
    return this->drawstyles[SoXtViewer::STILL];
  else
    return this->drawstyles[SoXtViewer::INTERACTIVE];
}

// Returns a boolean to indicate if the current drawstyle settings implies
// hidden line rendering.
SbBool
SoXtViewerP::drawAsHiddenLine(void) const
{
  return ((this->currentDrawStyle() == SoXtViewer::VIEW_HIDDEN_LINE) ? TRUE : FALSE);
}

// Returns a boolean to indicate if the current drawstyle settings
// implies wirefram overlay rendering.
SbBool
SoXtViewerP::drawAsWireframeOverlay(void) const
{
  return ((this->currentDrawStyle() == SoXtViewer::VIEW_WIREFRAME_OVERLAY) ? TRUE : FALSE);
}
...
// currently rendered.
void
SoXtViewerP::changeDrawStyle(SoXtViewer::DrawStyle style)
{
  ...
  if (style == SoXtViewer::VIEW_AS_IS) {
    this->drawstyleroot->whichChild = SO_SWITCH_NONE;
    return;
  }

  this->drawstyleroot->whichChild = SO_SWITCH_ALL;
  if ((style == SoXtViewer::VIEW_HIDDEN_LINE) ||
      (style == SoXtViewer::VIEW_WIREFRAME_OVERLAY)) {
...
#if 0 // debug
  SoDebugError::postInfo("SoXtViewer::changeDrawStyle",
                         "\n"
...
void
SoXtViewerP::setClippingPlanes(void)
{
  ...
  // either use an assert() (if it can't happen) or an SoXtSwap()
  ...
  if (this->camera->isOfType(SoPerspectiveCamera::getClassTypeId())) {
    ...
    if (this->autoclipstrategy == SoXtViewer::CONSTANT_NEAR_PLANE) {
      nearlimit = this->autoclipvalue;
    }
    else {
      assert(this->autoclipstrategy == SoXtViewer::VARIABLE_NEAR_PLANE);
  ...
  // modify the frustum.
  //
  // FIXME: quite the hack that SoXt needs to know about the
  ...
  // set the environment variable SOXT_DEBUG_CLIPPLANES):

#if SOXT_DEBUG
  static int debugoutputnearfar = -1;
  if (debugoutputnearfar == -1) {
    const char * env = SoAny::si()->getenv("SOXT_DEBUG_CLIPPLANES");
    debugoutputnearfar = (env && atoi(env) > 0) ? 1 : 0;
  }

  if (debugoutputnearfar == 1) { // debug
    SoDebugError::postInfo("SoXtViewer::setClippingPlanes",
                           "near, far: %f (%f), %f (%f)",
                           nearval, this->camera->nearDistance.getValue(),
...
void
SoXtViewerP::moveCameraScreen(const SbVec2f & screenpos)
{
  SoCamera * cam = PUBLIC(this)->getCamera();
  assert(cam);

  if (SOXT_DEBUG && 0) { // debug
    SoDebugError::postInfo("SoXtViewer::moveCameraScreen",
...
void
SoXtViewerP::interactivestartCB(void *, SoXtViewer * thisp)
{
  // In interactive buffer mode, doublebuffering is used during interaction.
  if (PRIVATE(thisp)->buffertype == SoXtViewer::BUFFER_INTERACTIVE) {
    PRIVATE(thisp)->localsetbuffertype = TRUE;
    thisp->SoXtRenderArea::setDoubleBuffer(TRUE);
    PRIVATE(thisp)->localsetbuffertype = FALSE;
  }

  // Use the dynamic drawstyle.
  if (!PRIVATE(thisp)->drawInteractiveAsStill())
    PRIVATE(thisp)->changeDrawStyle(PRIVATE(thisp)->drawstyles[SoXtViewer::INTERACTIVE]);
}

// Called when viewer goes out of interactive mode and into "frozen"
// mode.
void
SoXtViewerP::interactiveendCB(void *, SoXtViewer * thisp)
{
  // In interactive buffer mode, doublebuffering is used during
  // interaction, singelbuffering while the camera is static.
  if (PRIVATE(thisp)->buffertype == SoXtViewer::BUFFER_INTERACTIVE) {
    PRIVATE(thisp)->localsetbuffertype = TRUE;
    thisp->SoXtRenderArea::setDoubleBuffer(FALSE);
    PRIVATE(thisp)->localsetbuffertype = FALSE;
  }

  // Back to static drawstyle.
  if (!PRIVATE(thisp)->drawInteractiveAsStill())
    PRIVATE(thisp)->changeDrawStyle(PRIVATE(thisp)->drawstyles[SoXtViewer::STILL]);
}

// Called repeatedly during the seek animation.
void
SoXtViewerP::seeksensorCB(void * data, SoSensor * s)
{
  SbTime currenttime = SbTime::getTimeOfDay();

  SoXtViewer * thisp = (SoXtViewer *)data;
...
void
SoXtViewerP::resetFrameCounter(void)
{
  this->framecount = 0;
  for (int i = 0; i < SoXtViewerP::FRAMESARRAY_SIZE; i++)
...
SbVec2f
SoXtViewerP::addFrametime(const double ft)
{
  this->framecount++;
  ...
  float coinfps =
    this->totalcoin / SoXtMin(this->framecount, (int) FRAMESARRAY_SIZE);
  ...
  float drawfps =
    this->totaldraw / SoXtMin(this->framecount, (int) FRAMESARRAY_SIZE);
...
void
SoXtViewerP::recordFPS(const double rendertime)
{
...
SOXT_OBJECT_ABSTRACT_SOURCE(SoXtViewer);
...
*/
/*!
  \var SoXtViewer::Type SoXtViewer::BROWSER
...
*/
/*!
  \var SoXtViewer::Type SoXtViewer::EDITOR
  ...
  So if you want to avoid having the SoXtViewer class muck about
  with your supplied scenegraph, set the type-flag to
  SoXtViewer::BROWSER instead.
*/
...
/*!
  ...
  SoXtViewer::setDrawStyle() call. Decides the effect of the second
  argument.

  \sa SoXtViewer::setDrawStyle(), SoXtViewer::DrawStyle
*/
/*!
  \var SoXtViewer::DrawType SoXtViewer::STILL

  If this value is passed as the first argument of
  SoXtViewer::setDrawStyle(), the second argument decides which
...
*/
/*!
  \var SoXtViewer::DrawType SoXtViewer::INTERACTIVE
...
/*!
  ...
  \sa SoXtViewer::setDrawStyle(), SoXtViewer::DrawType
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_AS_IS

  Normal rendering, draws all scene geometry in it's original style.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_HIDDEN_LINE
...
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_WIREFRAME_OVERLAY

  Render the scene as normal, but overlay a set of lines showing the
  contours of all polygons.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_NO_TEXTURE

  Render scene without textures.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_LOW_COMPLEXITY
...
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_LINE

  View all polygon geometry in wireframe mode.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_POINT

  Render only the vertex positions of the geometry.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_BBOX
  ...
  this mode for the SoXtViewer::INTERACTIVE DrawType.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_LOW_RES_LINE

  Render as wireframe and don't bother with getting them rendered
  correctly in depth.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_LOW_RES_POINT

  Render as vertex points and don't bother with getting them rendered
  correctly in depth.
*/
/*!
  \var SoXtViewer::DrawStyle SoXtViewer::VIEW_SAME_AS_STILL

  Always render a scene with an animating camera (ie
  SoXtViewer::INTERACTIVE DrawType) in the same manner as scene
  with a still camera.
*/
...
*/
/*!
  \var SoXtViewer::BufferType SoXtViewer::BUFFER_SINGLE
  Change underlying OpenGL canvas to be single-buffered.
*/
/*!
  \var SoXtViewer::BufferType SoXtViewer::BUFFER_DOUBLE
  Change underlying OpenGL canvas to be double-buffered.
*/
/*!
  \var SoXtViewer::BufferType SoXtViewer::BUFFER_INTERACTIVE
  ...
  back buffer in the default SoXtViewer::BUFFER_DOUBLE mode.
*/
...
static SoGroup *
get_parent_of_node(SoXtViewerP * pimpl, SoNode * root, SoNode * node)
{
...
/*!
  ...
  SoXtRenderArea, so see the documentation for our parent
  ...
  programmer, but if you want to make sure the SoXtViewer class
  doesn't muck about with your supplied scenegraph, set the type-flag
  to SoXtViewer::BROWSER.  (This "feature" of the viewer is
...
{
  PRIVATE(this) = new SoXtViewerP(this);
  ...
  PRIVATE(this)->seeksensor = new SoTimerSensor(SoXtViewerP::seeksensorCB, this);
  ...
  this->addStartCallback(SoXtViewerP::interactivestartCB);
  this->addFinishCallback(SoXtViewerP::interactiveendCB);
...
*/
void
SoXtViewer::setCamera(SoCamera * cam)
{
...
/*!
  ...
  meant as a note for developers extending the SoXt library, as
...
*/
SoCamera *
SoXtViewer::getCamera(void) const
{
  // This impossible to miss reminder was inserted so we don't
  // accidentally let an So* v2 slip out the door without fixing this
  // API design flaw. 20030903 mortene.
#if (SOXT_MAJOR_VERSION == 2)
#error This is a reminder: when jumping to version 2 of an So* toolkit, the SoXtViewer::getCamera() method should be made virtual.
#endif // version = 2
...
void
SoXtViewer::setCameraType(SoType t)
{
  if (PRIVATE(this)->camera && 
...
#if SOXT_DEBUG
    SoDebugError::postWarning("SoXtViewer::setCameraType",
  ...
  if (PRIVATE(this)->camera != NULL) {
    ...
    if (newisperspective) {
      SoXtViewerP::convertOrtho2Perspective((SoOrthographicCamera *)PRIVATE(this)->camera,
                                               (SoPerspectiveCamera *)newcamera);
    }
    else {
      SoXtViewerP::convertPerspective2Ortho((SoPerspectiveCamera *)PRIVATE(this)->camera,
...
SoType
SoXtViewer::getCameraType(void) const
{
...
*/
void
SoXtViewer::saveHomePosition(void)
{
...
*/
void
SoXtViewer::resetToHomePosition(void)
{
  if (!PRIVATE(this)->camera) { return; } // probably a scene-less viewer
  ...
  if (t == s) {
    ...
    // detail in SoXtViewer::saveHomePosition().
  ...
  // handle common case #1
    ...
    SoXtViewerP::convertPerspective2Ortho((SoPerspectiveCamera *)PRIVATE(this)->storedcamera,
  ...
  // handle common case #2
    ...
    SoXtViewerP::convertOrtho2Perspective((SoOrthographicCamera *)PRIVATE(this)->storedcamera,
...
void
SoXtViewer::setHeadlight(SbBool on)
{
...
SbBool
SoXtViewer::isHeadlight(void) const
{
...
SoDirectionalLight *
SoXtViewer::getHeadlight(void) const
{
...
*/
void
SoXtViewer::setDrawStyle(SoXtViewer::DrawType type,
                            SoXtViewer::DrawStyle style)
{
  if (SOXT_DEBUG) {
    if ((type != STILL) && (type != INTERACTIVE)) {
      SoDebugError::postWarning("SoXtViewer::setDrawStyle",
...
SoXtViewer::DrawStyle
SoXtViewer::getDrawStyle(const SoXtViewer::DrawType type) const
{
  if (SOXT_DEBUG) {
    if ((type != STILL) && (type != INTERACTIVE)) {
      SoDebugError::postWarning("SoXtViewer::setDrawStyle",
...
/*!
  ...
  SoXtViewer::BUFFER_SINGLE, \c SoXtViewer::BUFFER_DOUBLE and \c
  SoXtViewer::BUFFER_INTERACTIVE.

  (With a buffer type of \c SoXtViewer::BUFFER_INTERACTIVE, the
  viewer will render with doublebuffering during user interaction and
  with single buffering otherwise.)

  Default is \c SoXtViewer::BUFFER_DOUBLE.
...
SoXtViewer::setBufferingType(SoXtViewer::BufferType type)
{
  ...
  if (type != BUFFER_SINGLE &&
    ...
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::setBufferingType",
...
SoXtViewer::getBufferingType(void) const
{
...
*/
void
SoXtViewer::setViewing(SbBool enable)
{
  if (PRIVATE(this)->viewingflag == enable) {
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::setViewing",
                                "unnecessary called");
    }
    return;
  }
...
*/
SbBool
SoXtViewer::isViewing(void) const
{
...
void
SoXtViewer::setCursorEnabled(SbBool on)
{
...
SbBool
SoXtViewer::isCursorEnabled(void) const
{
...
/*!
  ...
  possible to use the SoXtViewer::setAutoClippingStrategy() method
...
void
SoXtViewer::setAutoClipping(SbBool enable)
{
  if (SOXT_DEBUG) {
    if (PRIVATE(this)->adjustclipplanes == enable) {
      SoDebugError::postWarning("SoXtViewer::setAutoClipping",
                                "unnecessary called");
      return;
    }
  }
...
/*!
  ...
  calculated by the SoXtViewer code. The callback can then adjust
...
*/
void 
SoXtViewer::setAutoClippingStrategy(const AutoClippingStrategy strategy,
                                       const float value,
                                       SoXtAutoClippingCB * cb,
                                       void * cbuserdata)
{
  ...
  if (PRIVATE(this)->autoclipstrategy == VARIABLE_NEAR_PLANE) {
    ...
    float v = SoXtClamp(value, 0.0f, 1.0f); // just in case
    v *= 0.8f;
...
SbBool
SoXtViewer::isAutoClipping(void) const
{
...
void
SoXtViewer::setStereoViewing(SbBool enable)
{
...
SbBool
SoXtViewer::isStereoViewing(void) const
{
...
/*!
  \enum SoXtViewer::StereoType

  Contains list of supported stereo rendering techniques.

  \sa SoXtViewer::setStereoType()
*/
/*!
  \var SoXtViewer::StereoType SoXtViewer::STEREO_NONE

  Use monoscopic rendering.
*/
/*!
  \var SoXtViewer::StereoType SoXtViewer::STEREO_ANAGLYPH
...
*/
/*!
  \var SoXtViewer::StereoType SoXtViewer::STEREO_QUADBUFFER
...
*/
/*!
  \var SoXtViewer::StereoType SoXtViewer::STEREO_INTERLEAVED_ROWS
...
*/
/*!
  \var SoXtViewer::StereoType SoXtViewer::STEREO_INTERLEAVED_COLUMNS

  Same basic technique as SoXtViewer::STEREO_INTERLEAVED_ROWS, only
...
/*!
  ...
  SoXtViewer::setStereoViewing() and
  SoXtGLWidget::setQuadBufferStereo() functions.

  The default is to do monoscopic rendering, i.e. the default
  SoXtViewer::StereoType value is SoXtViewer::STEREO_NONE.

  \sa SoXtViewer::StereoType
  \since SoXt 1.2
*/
SbBool
SoXtViewer::setStereoType(SoXtViewer::StereoType s)
{
...
/*!
  ...
  SoXtViewer::STEREO_NONE if monoscopic).
*/
SoXtViewer::StereoType
SoXtViewer::getStereoType(void) const
{
  ...
  if (!PRIVATE(this)->stereotypesetexplicit) {
    if (this->isQuadBufferStereo()) {
      PRIVATE(this)->stereotype = SoXtViewer::STEREO_QUADBUFFER;
    }
    else if (this->isStereoViewing()) {
      PRIVATE(this)->stereotype = SoXtViewer::STEREO_ANAGLYPH;
    }
  }
...
/*!
  ...
  \sa SoXtViewer::StereoType, setStereoType()
*/
void
SoXtViewer::setAnaglyphStereoColorMasks(const SbBool left[3], const SbBool right[3])
{
...
*/
void
SoXtViewer::getAnaglyphStereoColorMasks(SbBool left[3], SbBool right[3])
{
  for (unsigned int i = 0; i < 3; i++) {
...
*/
void
SoXtViewer::setStereoOffset(const float dist)
{
...
*/
float
SoXtViewer::getStereoOffset(void) const
{
...
void
SoXtViewer::setDetailSeek(const SbBool on)
{
  if (SOXT_DEBUG) {
    if (PRIVATE(this)->seektopoint == on) {
      SoDebugError::postWarning("SoXtViewer::setDetailSeek",
                                "unnecessary called");
      return;
    }
  }
...
SbBool
SoXtViewer::isDetailSeek(void) const
{
...
void
SoXtViewer::setSeekTime(const float seconds)
{
  if (seconds < 0.0f) {
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::setSeekTime",
...
float
SoXtViewer::getSeekTime(void) const
{
...
void
SoXtViewer::addStartCallback(SoXtViewerCB * func, void * data)
{
...
void
SoXtViewer::removeStartCallback(SoXtViewerCB * func, void * data)
{
...
void
SoXtViewer::addFinishCallback(SoXtViewerCB * func, void * data)
{
...
void
SoXtViewer::removeFinishCallback(SoXtViewerCB * func, void * data)
{
...
void
SoXtViewer::setDoubleBuffer(const SbBool on)
{
...
void
SoXtViewer::setSceneGraph(SoNode * root)
{
  if ((root != NULL) && (root == PRIVATE(this)->scenegraph)) {
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::setSceneGraph",
                                "called with the same root as already set");
    }
    return;
  }

  // If the SoXtRenderArea hasn't yet set up its pointer to the
  // SoXtViewer "viewer root" (i.e. the viewer-generated root above
...
#if 0 // debug
  SoDebugError::postInfo("SoXtViewer::setSceneGraph",
  ...
  if (!scenecamera) {
    ...
    if (PRIVATE(this)->type == SoXtViewer::BROWSER) {
      PRIVATE(this)->sceneroot->insertChild(scenecamera, 1);
    }
    else { // PRIVATE(this)->type == SoXtViewer::EDITOR
...
*/
void
SoXtViewer::setSeekMode(SbBool enable)
{
  if (SOXT_DEBUG) {
...
*/
SbBool
SoXtViewer::isSeekMode(void) const
{
...
*/
SbBool
SoXtViewer::seekToPoint(const SbVec2s screenpos)
{
  if (! PRIVATE(this)->camera)
    return FALSE;
...
void
SoXtViewerP::setStereoEye(SoCamera * thecamera,
                             const SoXtViewerP::Eye eye,
                             SoXtViewerP::StereoData & s) const
{
...
void
SoXtViewerP::initStencilBufferForInterleavedStereo(void)
{
  ...
  SoXtViewer::StereoType s = PUBLIC(this)->getStereoType();
  assert((s == SoXtViewer::STEREO_INTERLEAVED_ROWS) ||
         (s == SoXtViewer::STEREO_INTERLEAVED_COLUMNS));
  ...
  const SbBool fillmask = allocnewmask || (this->stereostenciltype != s) ||
    ((s == SoXtViewer::STEREO_INTERLEAVED_ROWS) && (oldsize[0] != newsize[0]));
  ...
  if (fillmask) {
    ...
    if (s == SoXtViewer::STEREO_INTERLEAVED_COLUMNS) {
...
SoXtViewer::actualRedraw(void)
{
  ...
  const SoXtViewer::StereoType stereotype = this->getStereoType();

  if (stereotype != SoXtViewer::STEREO_NONE) {
    const SbColor bgcol = this->getSceneManager()->getBackgroundColor();

    SoCamera * camera = this->getCamera();
    SoXtViewerP::StereoData tmpstorage;

    // Render left eye:

    PRIVATE(this)->setStereoEye(camera, SoXtViewerP::LEFT, tmpstorage);
    ...
    PRIVATE(this)->setStereoEye(camera, SoXtViewerP::RIGHT, tmpstorage);
    ...
    PRIVATE(this)->setStereoEye(camera, SoXtViewerP::RESTORE, tmpstorage);
...
/*!
  ...
  SoXtViewer automatically adds callbacks to switch between still and
...
void
SoXtViewer::interactiveCountInc(void)
{
...
#if 0 // debug
  SoDebugError::postInfo("SoXtViewer::interactiveCountInc", "%d -> %d",
...
void
SoXtViewer::interactiveCountDec(void)
{
  ...
  if (SOXT_DEBUG) {
    if (PRIVATE(this)->interactionnesting <= 0) {
      SoDebugError::postWarning("SoXtViewer::interactiveCountDec",
                                "interaction count nesting went below zero. "
                                "This is due to an internal SoXt bug.");
    }
  }
...
int
SoXtViewer::getInteractiveCount(void) const
{
...
void
SoXtViewer::setSeekDistance(const float distance)
{
  if (distance <= 0.0f) {
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::setSeekDistance",
...
float
SoXtViewer::getSeekDistance(void) const
{
...
void
SoXtViewer::setSeekValueAsPercentage(const SbBool on)
{
  if (SOXT_DEBUG) {
    if ((on && this->isSeekValuePercentage()) ||
        (!on && !this->isSeekValuePercentage())) {
      SoDebugError::postWarning("SoXtViewer::setSeekDistanceAsPercentage",
...
SbBool
SoXtViewer::isSeekValuePercentage(void) const
{
...
/*!
  This method is obsoleted in Coin SoXt.
 */
void
SoXtViewer::computeSeekFinalOrientation(void)
{
  SOXT_OBSOLETED();
}
...
/*!
  ...
  Automatically calls SoXtViewer::setCameraType() so the change
  will immediately take place.
*/
void
SoXtViewer::toggleCameraType(void)
{
  SoType perspectivetype = SoPerspectiveCamera::getClassTypeId();
  SoType orthotype = SoOrthographicCamera::getClassTypeId();
 ...
 */
void
SoXtViewer::changeCameraValues(// virtual, protected
  SoCamera * camera)
{
  assert(camera != NULL);
  ...
  if (!cam) {
    if (SOXT_DEBUG) {
      SoDebugError::postWarning("SoXtViewer::changeCameraValues",
...
// doc in super
void
SoXtViewer::sizeChanged(const SbVec2s & size)
{
...
SbBool
SoXtViewer::processSoEvent(const SoEvent * const event)
{
  ...
  // to the SoXtRenderArea, which will pass it on to the
  // scenegraph.
...
void
SoXtViewer::addSuperimposition(SoNode * scene)
{
  if (PRIVATE(this)->superimpositions == NULL)
  ...
  if (PRIVATE(this)->searchaction->getPath() == NULL) {
    ...
    if (SOXT_DEBUG) {
      SoDebugError::postInfo("SoXtViewer::addSuperimposition",
...
void
SoXtViewer::removeSuperimposition(SoNode * scene)
{
  assert(scene);
  int idx = -1;
 ...
 error:
  if (SOXT_DEBUG) {
    SoDebugError::post("SoXtViewer::removeSuperimposition",
...
void
SoXtViewer::setSuperimpositionEnabled(SoNode * scene,
                                         const SbBool enable)
{
  int idx = -1;
  if (PRIVATE(this)->superimpositions == NULL) goto error;
 ...
 error:
  if (SOXT_DEBUG) {
    SoDebugError::post("SoXtViewer::setSuperimpositionEnabled",
                       "no such superimposition");
  }
  return;
}
...
SbBool
SoXtViewer::getSuperimpositionEnabled(SoNode * scene) const
{
  int idx = -1;
  if (PRIVATE(this)->superimpositions == NULL) goto error;
  idx = PRIVATE(this)->superimpositions->find(scene);
 ...
 error:
  if (SOXT_DEBUG) {
    SoDebugError::post("SoXtViewer::getSuperimpositionEnabled",
                       "no such superimposition");
  }
  return FALSE;
}
