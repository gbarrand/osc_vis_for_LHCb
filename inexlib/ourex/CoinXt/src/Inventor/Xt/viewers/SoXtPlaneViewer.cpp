...
/*!
  ...
  \brief The SoXtPlaneViewer class is for examining 3D models by moving the camera in orthogonal planes.
  \ingroup components viewers

  The SoXtPlaneViewer is a viewer that is useful for "CAD-style"
  ...
  The SoXtPlaneViewer also provides a user decoration's button for
...
/*!
  \fn SoXtPlaneViewer::SoXtPlaneViewer(Widget parent, const char * const name, SbBool embed, SoXtFullViewer::BuildFlag flag, SoXtViewer::Type type)

  The public constructor, to be used by application programmers who
  want to instantiate the plain, standard SoXtPlaneViewer.
*/
...
/*!
  \fn SoXtPlaneViewer::SoXtPlaneViewer(Widget parent, const char * const name, SbBool embed, SoXtFullViewer::BuildFlag flag, SoXtViewer::Type type, SbBool build)

  A protected constructor, to be used by application programmers who
  want to extend the SoXtPlaneViewer.
*/
SoXtPlaneViewer::SoXtPlaneViewer(Widget parent,
                                       const char * const name,
                                       SbBool embed,
...
SbBool
SoXtPlaneViewer::processSoEvent(const SoEvent * const ev)
{
#if SOXT_DEBUG && 0 // debug
  SoDebugError::postInfo("SoXtPlaneViewer::processSoEvent",
                          "[invoked], event '%s' "
  ...
  const SbVec2f posn((float)pos[0] / (float) SoXtMax(size[0], (short int)1),
                     (float)pos[1] / (float) SoXtMax(size[1], (short int)1));
  ...
  if (type.isDerivedFrom(SoLocation2Event::getClassTypeId())) {
    ...
    PRIVATE(this)->setPointerLocation(pos);
    const SbVec2f prevn((float) PRIVATE(this)->pointer.then[0] / (float) SoXtMax(size[0], (short int)1),
                        (float) PRIVATE(this)->pointer.then[1] / (float) SoXtMax(size[1], (short int)1));
...
// documented in superclass
void
SoXtPlaneViewer::setSeekMode(SbBool on)
{
#if SOXT_DEBUG
  if (!!on == !!this->isSeekMode()) {
    SoDebugError::postWarning("SoXtPlaneViewer::setSeekMode",
...
// Documented in superclass.
void
SoXtPlaneViewer::setCursorEnabled(SbBool enable)
{
...
// Documented in superclass.
const char *
SoXtPlaneViewer::getDefaultWidgetName(void) const
{
...
// Documented in superclass.
const char *
SoXtPlaneViewer::getDefaultTitle(void) const
{
  return "Plane Viewer";
}
...
// Documented in superclass.
const char *
SoXtPlaneViewer::getDefaultIconTitle(void) const
{
  return "Plane Viewer";
}
...
// Documented in superclass.
void
SoXtPlaneViewer::bottomWheelStart(void)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::bottomWheelMotion(float value)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::bottomWheelFinish(void)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::leftWheelStart(void)
{
  PRIVATE(this)->changeMode(SoGuiPlaneViewerP::TRANSLATE_MODE);
}
...
// Documented in superclass.
void
SoXtPlaneViewer::leftWheelMotion(float value)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::leftWheelFinish(void)
{
  PRIVATE(this)->changeMode(SoGuiPlaneViewerP::IDLE_MODE);
}
...
// Documented in superclass.
void
SoXtPlaneViewer::rightWheelStart(void)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::rightWheelMotion(float value)
{
...
// Documented in superclass.
void
SoXtPlaneViewer::rightWheelFinish(void)
{
  PRIVATE(this)->changeMode(SoGuiPlaneViewerP::IDLE_MODE);
}
...
// Documented in superclass.
void
SoXtPlaneViewer::afterRealizeHook(void)
{
  PRIVATE(this)->setCursorRepresentation(PRIVATE(this)->mode);
...
SoGuiPlaneViewerP::SoGuiPlaneViewerP(SoXtPlaneViewer * publ)
{
  PUBLIC(this) = publ;
}
...
SoGuiPlaneViewerP::commonConstructor(void)
{
  ...
  static const char * superimposed = {
    ...
    ""
    "Separator {"
    "  DEF soxt->orthocam OrthographicCamera {"
    "    height 1"
    "    nearDistance 0"
    "    farDistance 1"
    "  }"
    ...
    "    }"
    "    BaseColor { rgb 1 1 1 }"
    "    DEF soxt->style0 DrawStyle { }" // lineWidth set in code
    "    DEF soxt->mainline IndexedLineSet {" // fat line (exterior)
    "      coordIndex ["
    "        0, 1, -1, 1, 2, -1, 1, 3, -1"
    "      ]"
    "    }"
    "    BaseColor { rgb 0.5 0.5 0.5 }"
    "    DEF soxt->style1 DrawStyle { }" // lineWidth set in code
    "    USE soxt->mainline" // thin line (interior)
    "  }"
    "}"
  };
  ...
  this->super.coords = (SoCoordinate3 *)
    get_scenegraph_node(&s, this->superimposition, "soxt->geometry");
  this->super.camera = (SoOrthographicCamera *)
    get_scenegraph_node(&s, this->superimposition, "soxt->orthocam");
...
SoGuiPlaneViewerP::updateAnchorScenegraph(void) const
{
  ...
  if (first) {
    ...
    SoDrawStyle * ds0 = (SoDrawStyle *)
      get_scenegraph_node(&s, this->superimposition, "soxt->style0");
    SoDrawStyle * ds1 = (SoDrawStyle *)
      get_scenegraph_node(&s, this->superimposition, "soxt->style1");

    // Draw a thinner line on top of a fat line, to make an outline.
    ds0->lineWidth = SoXtMin(5.0f, range[1]);
    ds1->lineWidth = SoXtMax(3.0f, range[0]);
  }
...
{
#if SOXT_DEBUG && 0 // debug
  SoDebugError::postInfo("SoXtPlaneViewer::setCursorRepresentation",
                         "mode==%d", mode);
#endif // debug
