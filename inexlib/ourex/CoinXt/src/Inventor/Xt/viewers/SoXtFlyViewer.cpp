...
/*!
  ...
  \brief The SoXtFlyViewer class implements controls for moving
...
// duplication with some of the code in SoXtConstrainedViewerP, for
// example. 20021017 rolvs
class SoXtFlyViewerP {
public:
  SoXtFlyViewerP(SoXtFlyViewer * owner);
  ~SoXtFlyViewerP();
...
#define SOXT_MIN_STEP     0.2f
#define SOXT_INC_FACTOR   1.2f
#define SOXT_MAX_SPEED   20.0f
...
SoXtFlyViewerP::SoXtFlyViewerP( SoXtFlyViewer * owner )
{
  this->searcher = NULL;
  this->publ = owner;
...
SoXtFlyViewerP::~SoXtFlyViewerP(void)
{
...
void
SoXtFlyViewerP::constructor(SbBool build)
{
  ...
  static const char * superimposed[] = {
    ...
    "      ]",
    "    }",
    "    DEF soxt->maxspeedswitch Switch {",
    "      whichChild -3",
    // max speed indicator
    "      Material {",
    "        emissiveColor 1 0 0",
    ...
    "    DEF soxt->currentspeedswitch Switch {",
    "      whichChild -3",
    "      Material {",
    "        emissiveColor 0 0 1",
    "      }",
    ...
    "  }",
    // cross
    "  DEF soxt->crossswitch Switch {",
    "    whichChild -1",
    "    DEF soxt->crossposition Translation {",
    "      translation 0 0 0",
    "    }",
    "    DEF soxt->crossscale Scale {",
    "      scaleFactor 1 1 1",
    "    }",
    "    BaseColor {",
    "      rgb 1 0 0",
  ...
  this->sscale = (SoScale *)
    this->getSuperimpositionNode("soxt->scale");
  this->stranslation = (SoTranslation *)
    this->getSuperimpositionNode("soxt->translation");
  this->sgeometry = (SoCoordinate3 *)
    this->getSuperimpositionNode("soxt->geometry");
  this->smaxspeedswitch = (SoSwitch *)
    this->getSuperimpositionNode("soxt->maxspeedswitch");
  this->scurrentspeedswitch = (SoSwitch *)
    this->getSuperimpositionNode("soxt->currentspeedswitch");
  this->crossswitch = (SoSwitch *)
    this->getSuperimpositionNode("soxt->crossswitch");
  this->crossposition = (SoTranslation *)
    this->getSuperimpositionNode("soxt->crossposition");
  this->crossscale = (SoScale *)
    this->getSuperimpositionNode("soxt->crossscale");

  SoCallback * cb = (SoCallback *)
    this->getSuperimpositionNode("soxt->callback");
  cb->setCallback(SoXtFlyViewerP::superimposition_cb, this);
...
void
SoXtFlyViewerP::dolly(const float delta) const
{
...
void
SoXtFlyViewerP::setMode(ViewerMode newmode)
{
...
SoNode *
SoXtFlyViewerP::getSuperimpositionNode(const char * name)
{
  if (! this->searcher)
...
SbBool
SoXtFlyViewerP::processKeyboardEvent( const SoKeyboardEvent * const ke )
{
  assert( ke != NULL );
  switch (ke->getState()) {
  case SoButtonEvent::UP:
	...
	SbBool uppickmode =
	  this->getMode() == SoXtFlyViewerP::WAITING_FOR_UP_PICK;
	this->setMode(uppickmode ? SoXtFlyViewerP::FLYING :
			       SoXtFlyViewerP::WAITING_FOR_UP_PICK);
...
#if SOXT_DEBUG
	SoDebugError::post("SoXtFlyViewerP::processKeyboardEvent",
    ...
    case SoKeyboardEvent::LEFT_CONTROL:
      ...
      if (this->lctrldown < 0) {
#if SOXT_DEBUG
	SoDebugError::post("SoXtFlyViewerP::processKyeboardEvent",
  ...
  case SoButtonEvent::DOWN:
    ...
    case SoKeyboardEvent::RIGHT_CONTROL:
      ...
      if (this->rctrldown > 2) {
#if SOXT_DEBUG
	SoDebugError::post("SoXtFlyViewer::processSoEvent",
  ...
  if ((this->getMode() == SoXtFlyViewerP::FLYING) &&
      (this->lctrldown || this->rctrldown)) {
    this->setMode(SoXtFlyViewerP::TILTING);
  ...
  } else if ((this->getMode() == SoXtFlyViewerP::TILTING) &&
	     !this->lctrldown && !this->rctrldown) {
    this->setMode(SoXtFlyViewerP::FLYING);
...
SbBool
SoXtFlyViewerP::processMouseButtonEvent( const SoMouseButtonEvent * const me )
{
  assert( me != NULL );
  
  // FIXME: only for fly mode
  switch (this->getMode()) {
  case SoXtFlyViewerP::WAITING_FOR_UP_PICK:
...
SbBool
SoXtFlyViewerP::processLocation2Event( const SoLocation2Event * const lev )
{  
  this->mouseloc = lev->getPosition();

  if (this->getMode() == SoXtFlyViewerP::TILTING) {
...
void
SoXtFlyViewerP::superimpositionevent(SoAction * action)
{
...
void
SoXtFlyViewerP::superimposition_cb(void * closure, SoAction * action)
{
  assert(closure != NULL);
  ((SoXtFlyViewerP *) closure)->superimpositionevent(action);
}

void
SoXtFlyViewerP::updateSpeedIndicator(void)
{
  ...
  points[10][0] = this->maxspeed / (SOXT_MAX_SPEED / 0.8f);
  points[11][0] = this->maxspeed / (SOXT_MAX_SPEED / 0.8f);
  points[14][0] = this->currentspeed / (SOXT_MAX_SPEED / 0.8f);
  points[15][0] = this->currentspeed / (SOXT_MAX_SPEED / 0.8f);
...
float SoXtFlyViewerP::calculateChangeInTime()
{
  SbTime thisrender; 
...
void SoXtFlyViewerP::updateCurrentSpeed( float dt )
{
...
void SoXtFlyViewerP::updateCameraPosition( SoCamera * camera, 
                                              float current_speed,
                                              float dt )
{
  assert( camera != NULL );
...
void SoXtFlyViewerP::updateCameraOrientation( SoCamera * camera, 
                                                 float d_tilt, 
                                                 float d_pan, 
                                                 float dt )
{
...
void
SoXtFlyViewerP::incrementMaxSpeed()
{
...
void
SoXtFlyViewerP::decrementMaxSpeed()
{	
  this->max_speed_factor--;
  updateMaxSpeed();
}
...
void SoXtFlyViewerP::updateSpeedScalingFactor()
{
...
void SoXtFlyViewerP::stopMoving()
{
  maxspeed = 0;
  currentspeed = 0; 
  max_speed_factor = 0;
...
void SoXtFlyViewerP::updateMaxSpeed()
{
  ...
  this->maxspeed = 
    this->max_speed_factor 
    * float(pow( SOXT_INC_FACTOR, abs( this->max_speed_factor ) ))
    * this->speed_scaling_factor;

  if( this->maxspeed > SOXT_MAX_SPEED )
    this->maxspeed = SOXT_MAX_SPEED;
  else if( this->maxspeed < -1*SOXT_MAX_SPEED )
    this->maxspeed = -1*SOXT_MAX_SPEED;
}
...
void
SoXtFlyViewerP::updateCursorRepresentation(void)
{
  ...
  case SoXtFlyViewerP::WAITING_FOR_SEEK:
    PUBLIC(this)->setComponentCursor(SoXtCursor(SoXtCursor::CROSSHAIR));
    break;

  case SoXtFlyViewerP::WAITING_FOR_UP_PICK:
    PUBLIC(this)->setComponentCursor(SoXtCursor(SoXtCursor::UPARROW));
    break;

  case SoXtFlyViewerP::TILTING:
...
/*!
  Public constructor.
*/
SoXtFlyViewer::SoXtFlyViewer(Widget parent,
                                   const char * name, 
                                   SbBool embed, 
...
{
  PRIVATE(this) = new SoXtFlyViewerP(this);
...
// doc in super
void
SoXtFlyViewer::setViewing(SbBool enable)
{
...
// doc in super
void
SoXtFlyViewer::resetToHomePosition(void)
{
  PRIVATE(this)->stopMoving();
...
// doc in super
void
SoXtFlyViewer::setCamera(SoCamera * camera)
{
  PRIVATE(this)->stopMoving();
...
// doc in super
void
SoXtFlyViewer::setCursorEnabled(SbBool enable)
{
...
// doc in super
const char *
SoXtFlyViewer::getDefaultWidgetName(void) const
{
...
// doc in super
const char *
SoXtFlyViewer::getDefaultTitle(void) const
{
...
// doc in super
const char *
SoXtFlyViewer::getDefaultIconTitle(void) const
{
...
SbBool
SoXtFlyViewer::processSoEvent(const SoEvent * const event)
{
  // FIXME: Refactor the event-handling so that it uses the same
  // strategy as in SoXtExaminerViewer, where the event-handler
  ...
  // SoXt*FlyViewer::processEvent that should go in to the
...
// doc in super
void
SoXtFlyViewer::setSeekMode(SbBool enable)
{
  // Note: this method is almost identical to the setSeekMode() in the
  // SoXtExaminerViewer, so migrate any changes.

#if SOXT_DEBUG
  if (enable == this->isSeekMode()) {
    SoDebugError::postWarning("SoXtFlyViewer::setSeekMode",
  ...
  PRIVATE(this)->setMode(enable ? SoXtFlyViewerP::WAITING_FOR_SEEK :
                            SoXtFlyViewerP::FLYING);
}
...
// doc in super
void
SoXtFlyViewer::actualRedraw(void)
{
  if (!this->isViewing()) {
...
// doc in super
void
SoXtFlyViewer::rightWheelMotion(float value)
{
...
// doc in super
void
SoXtFlyViewer::afterRealizeHook(void)
{
  PRIVATE(this)->updateCursorRepresentation();
