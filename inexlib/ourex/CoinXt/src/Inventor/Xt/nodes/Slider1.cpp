...
/*!
  \class SoGuiSlider1 Inventor/Xt/nodes/SoGuiSlider1.h
...
SoGuiSlider1::getValueAsColor(void) const
{
  ...
  float r = SoXtClamp(PRIVATE(this)->mincolor[0] + (PRIVATE(this)->maxcolor[0] - PRIVATE(this)->mincolor[0]) * factor, 0.0f, 1.0f);
  float g = SoXtClamp(PRIVATE(this)->mincolor[1] + (PRIVATE(this)->maxcolor[1] - PRIVATE(this)->mincolor[1]) * factor, 0.0f, 1.0f);
  float b = SoXtClamp(PRIVATE(this)->mincolor[2] + (PRIVATE(this)->maxcolor[2] - PRIVATE(this)->mincolor[2]) * factor, 0.0f, 1.0f);
  ...
  if ( PRIVATE(this)->grabbing ) { // click-and-drag
    if ( event->isOfType(SoLocation2Event::getClassTypeId()) ) {
      ...
      if ( raypos[0] != -1.0f ) {
        ...
        float maxval = this->max.getValue();
        float imagval;
        if ( minval < maxval ) imagval = SoXtClamp(imagpos / this->size.getValue()[0], minval, maxval);
        else imagval = SoXtClamp(imagpos / this->size.getValue()[0], maxval, minval);
