...
/*!
  \class SoXtCursor
  \brief The SoXtCursor class is used to set cursors for GUI components.
  \ingroup misc
  ...
  SoXtComponent::setComponentCursor() and
  SoXtComponent::setWidgetCursor() methods.
*/
...
/*!
  \var SoXtCursor::Shape SoXtCursor::CUSTOM_BITMAP
...
/*!
  \var SoXtCursor::Shape SoXtCursor::DEFAULT

  Using the native Window system's default cursor.
*/

/*!
  \var SoXtCursor::Shape SoXtCursor::BUSY

  A busy cursor.
*/

/*!
  \var SoXtCursor::Shape SoXtCursor::CROSSHAIR

  Two thin lines in a cross.
*/

/*!
  \var SoXtCursor::Shape SoXtCursor::UPARROW
...
/*!
  \class SoXtCursor::CustomCursor
  \brief The SoXtCursor::CustomCursor class is used to specify bitmap data for SoXtCursor custom cursors.
*/

/*!
  \var SbVec2s SoXtCursor::CustomCursor::dim

  Size of cursor data. Can be anything from 1 pixel wide and high, up
  to maximum 32 pixels in each dimension.
*/
/*!
  \var SbVec2s SoXtCursor::CustomCursor::hotspot
...
*/
/*!
  \var unsigned char * SoXtCursor::CustomCursor::bitmap
...
*/  
/*!
  \var unsigned char * SoXtCursor::CustomCursor::mask

  See doc of SoXtCursor::CustomCursor::bitmap.
*/
...
static SoXtCursor::CustomCursor zoom;
static SoXtCursor::CustomCursor pan;
static SoXtCursor::CustomCursor rotate;
static SoXtCursor::CustomCursor blank;
...
{
  this->commonConstructor(SoXtCursor::DEFAULT, NULL);
}
...
void
SoXtCursor::commonConstructor(const Shape shapearg, const CustomCursor * ccarg)
{ 
  this->shape = shapearg;
  this->cc = NULL;
...
/*!
  ...
  cursor, SoXtCursor::CUSTOM_BITMAP is returned.
*/
SoXtCursor::Shape 
SoXtCursor::getShape(void) const
{
  return this->shape;
}
...
*/
void 
SoXtCursor::setShape(const Shape shapearg)
{
  this->shape = shapearg;
}
...
/*!
  ...
  Do not call this method unless SoXtCursor::getShape() returns
  SoXtCursor::CUSTOM_BITMAP.
*/
const SoXtCursor::CustomCursor &
SoXtCursor::getCustomCursor(void) const
{
...
*/
const SoXtCursor &
SoXtCursor::getZoomCursor(void)
{
  static SoXtCursor * zoomcursor = NULL;
  // FIXME: memory leak. 20011120 mortene.
  if (!zoomcursor) { zoomcursor = new SoXtCursor(&zoom); }
  return *zoomcursor;
}
...
*/
const SoXtCursor &
SoXtCursor::getPanCursor(void)
{
  static SoXtCursor * pancursor = NULL;
  // FIXME: memory leak. 20011120 mortene.
  if (!pancursor) { pancursor = new SoXtCursor(&pan); }
  return *pancursor;
}
...
*/
const SoXtCursor &
SoXtCursor::getRotateCursor(void)
{
  static SoXtCursor * rotatecursor = NULL;
  // FIXME: memory leak. 20011120 mortene.
  if (!rotatecursor) { rotatecursor = new SoXtCursor(&rotate); }
  return *rotatecursor;
}
...
*/
const SoXtCursor &
SoXtCursor::getBlankCursor(void)
{
  static SoXtCursor * blankcursor = NULL;
  // FIXME: memory leak. 20011120 mortene.
  if (!blankcursor) { blankcursor = new SoXtCursor(&blank); }
  return *blankcursor;
}
