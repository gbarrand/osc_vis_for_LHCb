...
public:
  ...
  SoXtMaterialEditor * api;

  void constructor(void);

  SoXtMaterialEditor::UpdateFrequency frequency;
...
SoXtMaterialEditor::SoXtMaterialEditor(Widget parent, const char * name, SbBool embed)
...
SoXtMaterialEditor::SoXtMaterialEditor(Widget parent, const char * const name, SbBool embed, SbBool build)
: inherited(parent, name, embed)
{
  this->internals = new MaterialEditorComponent;
  PRIVATE(this)->api = this;
...
SoXtMaterialEditor::~SoXtMaterialEditor(void)
{
...
void
SoXtMaterialEditor::attach(SoMaterial * material, int index)
{
  assert(PRIVATE(this)->editor);
...
void
SoXtMaterialEditor::attach(SoVRMLMaterial * material)
{
  assert(PRIVATE(this)->editor);
#ifdef HAVE_SOVRMLMATERIAL
  PRIVATE(this)->editor->material.setValue(material);
...
void
SoXtMaterialEditor::detach(void)
{
  PRIVATE(this)->editor->material.setValue(NULL);
}

SbBool
SoXtMaterialEditor::isAttached(void)
{
...
void
SoXtMaterialEditor::addMaterialChangedCallback(SoXtMaterialEditorCB * callback, void * closure)
{
...
void
SoXtMaterialEditor::removeMaterialChangedCallback(SoXtMaterialEditorCB * callback, void * closure)
{
  assert(PRIVATE(this)->callbacks);
  int i;
  for ( i = PRIVATE(this)->callbacks->getLength(); i > 0; i -= 2 ) {
    SoXtMaterialEditorCB * cb = (SoXtMaterialEditorCB *) (*(PRIVATE(this)->callbacks))[i-2];
...
void
SoXtMaterialEditor::setUpdateFrequency(SoXtMaterialEditor::UpdateFrequency frequency)
{
  PRIVATE(this)->frequency = frequency;
}

SoXtMaterialEditor::UpdateFrequency
SoXtMaterialEditor::getUpdateFrequency(void) const
{
  return PRIVATE(this)->frequency;
}

void
SoXtMaterialEditor::setMaterial(const SoMaterial & material)
{
...
void
SoXtMaterialEditor::setMaterial(const SoVRMLMaterial & material)
{
#ifdef HAVE_SOVRMLMATERIAL
  float h, s, v;
  h = 0.0f;
...
const SoMaterial &
SoXtMaterialEditor::getMaterial(void) const
{
  return *(PRIVATE(this)->material);
}

SbBool
SoXtMaterialEditor::isAttachedVRML(void)
{
...
SoGuiMaterialEditor *
SoXtMaterialEditor::getEditor(void) const
{
  assert(0);
  return NULL;
}
...
const char *
SoXtMaterialEditor::getDefaultWidgetName(void) const
{
  static const char widgetName[] = "SoXtMaterialEditor";
  return widgetName;
}

const char *
SoXtMaterialEditor::getDefaultTitle(void) const
{
...
const char *
SoXtMaterialEditor::getDefaultIconTitle(void) const
{
...
MaterialEditorComponent::coloreditoropen_cb(void * closure)
{
  ...
  me->coloreditor = new SoXtColorEditor;
  me->coloreditor->show();
