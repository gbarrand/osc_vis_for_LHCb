...
void SoPolyhedron::generateAlternateRep(SoAction*) {
  ...
  if(solid.getValue()==TRUE) {
    ...
    do {
      ...
      coordinate3->point.setValues(icoord,points.size(),points.data());
      icoord += points.size();

      normals.push_back(SbVec3f(unitNormal[0],unitNormal[1],unitNormal[2]));

      indexedFaceSet->coordIndex.setValues
        (iindex,(ipoint+1),coordIndex.data());
      iindex += ipoint+1;

    } while (notLastFace);

    normal->vector.setValues(0,normals.size(),normals.data());
    ...
    SoCoordinate3* coordinate3 = new SoCoordinate3;
    coordinate3->point.setValues(0,points.size(),points.data());
    separator->addChild(coordinate3);

    SoIndexedLineSet* indexedLineSet = new SoIndexedLineSet;
    indexedLineSet->coordIndex.setValues(0,coords.size(),coords.data());
