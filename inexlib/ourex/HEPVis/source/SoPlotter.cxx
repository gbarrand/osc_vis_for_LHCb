...
static float bins2DToFunc(const SbPlottableBins2D&,float,float);
...
#define DUMP_UPDATE_WHAT(aString) \
  if(verbose.getValue()==TRUE) SoDebugError::postInfo("SoPlotter","%lu : UPDATE WHAT : %s",(unsigned long)this,(aString));
...
void SoPlotter::doAction(SoAction* aAction){
  if(verbose.getValue()==TRUE) {
    SoDebugError::postInfo("SoPlotter::doAction","%lu (%d).",
      (unsigned long)this,sDoCounter++);
  }
...
SoStyle* SoPlotter::getWallStyle() const {
  return (SoStyle*)wallStyle.getValue();}
SoStyle* SoPlotter::getGridStyle() const {
  return (SoStyle*)gridStyle.getValue();}
SoStyle* SoPlotter::getInnerFrameStyle() const {
  return (SoStyle*)innerFrameStyle.getValue();}
SoStyle* SoPlotter::getInfosStyle() const {
  return (SoStyle*)infosStyle.getValue();}
SoTextStyle* SoPlotter::getTitleStyle() const {
...
#ifdef DEBUG
  printf("debug : SoPlotter::updateChildren : objn = %d ; bins1D = %d ; bins2D = %d ; points2D = %d ; points3D = %d ; function = %d\n",
  objn,bins1DNumber,bins2DNumber,points2DNumber,points3DNumber,functionNumber);
#endif
...
#define XY_DEPTH_DATA 0.25F
    ...
    if(f_points_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : XY : update points",(unsigned long)this);
      }
    ...
    if(f_function_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : XY : update function",(unsigned long)this);
      }
    ...
    if(f_bins_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : XYZ : update bins",(unsigned long)this);
      }
    ...
    if(f_points_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : XYZ : update points",(unsigned long)this);
      }
    ...
    if(f_function_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : XYZ : update function",(unsigned long)this);
      }
  ...
  if(f_innerFrame_Update==TRUE) {
    ...
    if(verbose.getValue()==TRUE) {
      SoDebugError::postInfo("SoPlotter::updateChildren",
      "%lu : update innerFrame : draw %d : shape %d",
      (unsigned long)this,drawInnerFrame,fShape);
    }
  ...
  if(fShape==XY) {
    if(f_xAxis_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : update X axis",(unsigned long)this);
      }
    ...
    if(f_yAxis_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : update Y axis",(unsigned long)this);
      }
    ...
    if(f_zAxis_Update==TRUE) {
      ...
      if(verbose.getValue()==TRUE) {
        SoDebugError::postInfo("SoPlotter::updateChildren",
        "%lu : update Z axis",(unsigned long)this);
      }
  ...
  if(f_wall_Update==TRUE) {
    ...
    if(verbose.getValue()==TRUE) {
      SoDebugError::postInfo("SoPlotter::updateChildren",
      "%lu : update wall",(unsigned long)this);
    }
  ...
  if(f_grid_Update==TRUE) {
    ...
    if(verbose.getValue()==TRUE) {
      SoDebugError::postInfo("SoPlotter::updateChildren",
      "%lu : update grid",(unsigned long)this);
    }
  ...
  if(f_title_Update==TRUE) {
    ...
    if(verbose.getValue()==TRUE) {
      SoDebugError::postInfo("SoPlotter::updateChildren",
      "%lu : update title",(unsigned long)this);
    }
  ...
  if(f_colorMapping_Update==TRUE) {
    ...
    if(verbose.getValue()==TRUE) {
      SoDebugError::postInfo("SoPlotter::updateChildren",
      "%lu : update colormapping",(unsigned long)this);
    }
...
void SoPlotter::updateSbAxes(){
  ...
  if(f_binsList.size()) { 
    ...
    if(f_functionList[0]->getDimension()==1) {
      SbPlottableFunction1D* func = 
        Cast(*(f_functionList[0]),SbPlottableFunction1D);
      ...
      if(yAxisAutomated.getValue()==TRUE) {
        ...
        if(problem==TRUE) {
          SoDebugError::postInfo("SoPlotter::updateSbAxes",
      ...
      SbPlottableFunction2D* func =
        Cast(*(f_functionList[0]),SbPlottableFunction2D);
...
inline SbBool SbSTREMPTY(const char* aString) {
...
SoDrawStyle* SoPlotter::getLineStyle(const SbStyle& aStyle){
  return fStyleCache->getLineStyle 
           (aStyle.getLinePattern(),(float)aStyle.getLineWidth());
}

SoDrawStyle* SoPlotter::getLineStyle(SoStyle& aStyle){
  return fStyleCache->getLineStyle
          (aStyle.linePattern.getValue(),(float)aStyle.lineWidth.getValue());
}
...
void SoPlotter::updateWall2D(){
  ...
  sep->addChild(fStyleCache->getMaterial
               (style->color.getValue(),
                style->transparency.getValue()));
...
void SoPlotter::updateGrid2D(){
  ...
  int pos = 0;
  int number =
    xAxisNode->tickNumber.getValue() + yAxisNode->tickNumber.getValue();
  if(number<=0) return;
...
,float aZZ
  ...
  char sid[128];
  ::sprintf(sid,"SbObject/0x%lx",
    (unsigned long)const_cast<SbPlottableBins1D*>(&aBins));
  ...
  if(aGraphicStyle.visible.getValue()==TRUE) {
   ...
   {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
    ...
    if(modeling==SbModeling_top_lines){
      ...
      if(oneNode) {
        repBins1D_xy_boxes_one(binsNode,aGraphicStyle,
                            bins,aBoxX,aBoxY,aZZ,SbString(sid));
      } else {
      ...
      if(oneNode) {
        repBins1D_xy_wire_boxes_one(binsNode,aGraphicStyle,barChart,
                                 bins,aBoxX,aBoxY,aZZ,SbString(sid));
      } else {
      ...
      repBins1D_xy_lines_one(binsNode,aGraphicStyle,
                          bins,aBoxX,aBoxY,aZZ,SbString(sid));
    } else if(modeling==SbModeling_curve){
      repBins1D_xy_curve_one(binsNode,aGraphicStyle,
                          bins,aBoxX,aBoxY,aZZ,SbString(sid));
    }
  
    SbHatchingPolicy hatching = 
      (SbHatchingPolicy)aGraphicStyle.hatching.getValue();
      ...
      if((hatching==SbHatchingRight)||((hatching==SbHatchingLeftAndRight))) {
        if(oneNode) {
          repHatch1D_xy_one(binsNode,aRightHatchStyle,barChart,
                         bins,aBoxX,aBoxY,aZZ+zHatchOffset,SbString(sid));
        } else {
          repHatch1D_xy(binsNode,aRightHatchStyle,barChart,
                     bins,aBoxX,aBoxY,aZZ+zHatchOffset);
        }
      }
 ...
 {// Errors :
  if(aErrorsStyle.visible.getValue()==TRUE) { 
    ...
    if(markerStyle==SbMarkerPlus)
      repErrorsPlus_xy(errorsNode,aErrorsStyle,
                    bins,aBoxX,aBoxY,bars,aZZ+zErrorOffset,SbString(sid));
    else 
      repErrorsI_xy(errorsNode,aErrorsStyle,
                 bins,aBoxX,aBoxY,bars,aZZ+zErrorOffset,SbString(sid));
 }}
}
...
static double bins2D_to_contour(double aX,double aY,void* aData) {
  ...
  return bins2DToFunc(*(func->fBins2D),(float)aX,(float)aY);
}
static double log_bins2D_to_contour(double aX,double aY,void* aData) {
  ...
  return TakeLog(bins2DToFunc(*(func->fBins2D),(float)aX,(float)aY));
}

void SoPlotter::updateBins2D_XY(
 const SbPlottableBins2D& aBins
,int aIndex
,SoStyle& aGraphicStyle
,const SbRepBox& aBoxX
,const SbRepBox& aBoxY
...
,float aZZ
  ...
  char sid[128];
  ::sprintf(sid,"SbObject/0x%lx",
    (unsigned long)const_cast<SbPlottableBins2D*>(&aBins));
  ...
  if( (modeling==SbModeling_curve)  || (modeling==SbModeling_filled_curve) ){
    ...
    CListContour listContour;
    //int nFir = 32;
    int nFir = 128;
    listContour.SetFirstGrid(nFir,nFir); //Default : 32,32 
    //int nSec = 256;
    int nSec = 512;
    listContour.SetSecondaryGrid(nSec,nSec); //Default : 256,256.
    ...
    if(zlog==TRUE)
      listContour.SetFieldFcn(log_bins2D_to_contour,(void*)&sbFunc);
    else
      listContour.SetFieldFcn(bins2D_to_contour,(void*)&sbFunc);

    listContour.CContour::Generate();
    if(!listContour.CompactStrips()) {
      SoDebugError::postInfo("SoPlotter::updateBins2D_XY",
       " CListContour::CompactStrips() : failure.");
    } else {
  
      if( (painting==SbPaintingByLevel) ||
          (painting==SbPaintingByValue) ){
        f_bins_cmaps[aIndex] = 
          new SbByValueColormap(aGraphicStyle.colorMapping.getValue());
        SbBool zlog = aBoxZ.fLog;
      ...
      if(modeling==SbModeling_filled_curve) {
        repContour_xy_filled(binsNode,
                             aGraphicStyle,
                             painting,*(f_bins_cmaps[aIndex]),
                             listContour,aBoxX,aBoxY,aBoxZ,aZZ,SbString(sid));
      } else {
 ...
 {for(int jbin=ynbin-1;jbin>=0;jbin--) {
    for(int ibin=xnbin-1;ibin>=0;ibin--) { 
      if(aBins.getBinNumberOfEntries(ibin,jbin)<=0) continue;
  ...
  char sid_bin[128];
  ::sprintf(sid_bin,"SbBin2D/0x%lx",(unsigned long)&aBins);
   ...
   {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
    ...
    if(oneNode) {
      repBins2D_xy_solid_one(binsNode,
                             aGraphicStyle,
                             painting,*(f_bins_cmaps[aIndex]),
                             bins,aBoxX,aBoxY,aBoxZ,SbString(sid));
    } else {
...
void SoPlotter::updateFunction_XY(
 const SbPlottableFunction1D& aFunction
,SoStyle& aGraphicStyle
,const SbRepBox& aBoxX
,const SbRepBox& aBoxY
,const SbRepBox& aBoxZ
,float aZZ
  ...
  char sid[128];
  ::sprintf(sid,"SbFunction1D/0x%lx",(unsigned long)&aFunction);
  ...
  if(modeling==SbModeling_points)  {
    ...
    if(ptn) {
      ...
      separator->addChild(fStyleCache->getMaterial
        (aGraphicStyle.color.getValue(),
         aGraphicStyle.transparency.getValue()));
      SoTools_addPointsToNode(separator,ptn,pts,
        (SbMarkerStyle)aGraphicStyle.markerStyle.getValue(),
        aGraphicStyle.markerSize.getValue());
      delete [] pts;
    }  
...
public:
  SoPlotter_dragger(SoCoordinate3& aCoord,int aIndex,float aScale,SoPlotter& aPlotter,SbPlottableFunction2D& aFunction,const SbRepBox& aBoxX,const SbRepBox& aBoxY,float aZZ):SoDragPoint2D(aCoord,aIndex,aScale),fPlotter(aPlotter),fFunction(aFunction),fBoxX(aBoxX),fBoxY(aBoxY),fZZ(aZZ){
...
,float aZZ
  ...
  if( (modeling==SbModeling_curve) ||
      (modeling==SbModeling_filled_curve) ) {
    ...
    if(zlog==TRUE) {
      listContour.SetFieldFcn(log_function_to_contour,(void*)&sbFunc);
    } else {
      listContour.SetFieldFcn(function_to_contour,(void*)&sbFunc);
    }

    listContour.CContour::Generate();
    if(!listContour.CompactStrips()) {
      SoDebugError::postInfo
        ("SoPlotter::updateFunction_XY",
         " CListContour::CompactStrips() : failure.");
    } else {
    ...
    if(editable) {
      ...
      for(int index=0;index<controls.size();index++) {
        ...
        SoPlotter_dragger* dragger = 
          new SoPlotter_dragger(*coordinate3,icontrol,scale,
            *this,const_cast<SbPlottableFunction2D&>(aFunction),
            aBoxX,aBoxY,aZZ);
    ...
    SoDebugError::postInfo("SoPlotter::updateFunction_XY",
        "modeling \"%s\" does not apply on Function2D in XY. Valid modelings ared curve, filled_curve and polygon.",modeling.getString());
  }
...
,float aZZ
  ...
  char sid[128];
  ::sprintf(sid,"SbObject/0x%lx",
    (unsigned long)const_cast<SbPlottablePoints2D*>(&aPoints));
 ...
 {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
  ...
  if(oneNode) {
    ...
    repPoints2D_xy(pointsNode,aGraphicStyle,points,aBoxX,aBoxY,0,SbString(sid));
  }

  } //end graphicStyle visible
}

void SoPlotter::updateBins2D_XYZ(
 const SbPlottableBins2D& aBins
,int aIndex
,SoStyle& aGraphicStyle
,const SbRepBox& aBoxX
...
,const SbRepBox& aBoxZ
  ...
  if(modeling==SbModeling_boxes){
   ...
   {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
    ...
    if(oneNode) {
      repBins2D_xyz_boxes_one(binsNode,
                          aGraphicStyle,
                          bins,aBoxX,aBoxY,aBoxZ,bmin,bmax,SbString(sid));
    } else {
      repBins2D_xyz_boxes(binsNode,
                aGraphicStyle,painting,*(f_bins_cmaps[aIndex]),
                bins,aBoxX,aBoxY,aBoxZ,bmin,bmax,SbString(sid_bin));
    }
    ...
    if(painting==SbPaintingByLevel) { //OpenPAW.
     ...
     {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
      ...
      if(oneNode) {       
        repTopFaces2D_xyz_one(binsNode,aGraphicStyle,
        ...
        if(cmap->colorn()<=0) {
          ...
          listContour.CContour::Generate();
          if(!listContour.CompactStrips()) {
            SoDebugError::postInfo("SoPlotter::updateBins2D_XYZ",
             " CListContour::CompactStrips() : failure.");
            ...
            repContour_xy_filled(binsNode,
                                 *gs,
                                 ...
                                 listContour,
                                 aBoxX,aBoxY,aBoxZ,ZZ,SbString(sid));
            gs->unref();
          }
...
,const SbRepBox& aBoxZ
  ...
  char sid[128];
  ::sprintf(sid,"SbObject/0x%lx",
    (unsigned long)const_cast<SbPlottablePoints3D*>(&aPoints));
 ...
 {int nlimit = aGraphicStyle.multiNodeLimit.getValue();
  ...
  if(oneNode) {
    ...
    repPoints3D_xyz
      (pointsNode,aGraphicStyle,points,aBoxX,aBoxY,aBoxZ,SbString(sid));
  }
...
,const SbRepBox& aBoxZ
  ...
  if(problem==TRUE) {
    SoDebugError::postInfo("SoPlotter::updateFunction_XYZ",
      " problem when getting some function value.");
  }
...
void SoPlotter::updateTitle(){
  ...
  if( (fontName.getLength() >=6) &&
    ...
    titleNode->addChild(
      fStyleCache->getLineStyle(graphicStyle->linePattern.getValue(),
                       (float)graphicStyle->lineWidth.getValue()));
  }
...
void SoPlotter::updateInnerFrame_XY(){
  ...
  innerFrameNode->addChild(fStyleCache->getMaterial
    (graphicStyle->color.getValue(),
     graphicStyle->transparency.getValue()));
...
void SoPlotter::updateInnerFrame_XYZ(){
  ...
  innerFrameNode->addChild(getLineStyle(*graphicStyle));
  innerFrameNode->addChild(fStyleCache->getMaterial
    (graphicStyle->color.getValue(),graphicStyle->transparency.getValue()));
  SbVec3f points[5];
...
,const SbString& aID
  ...
  for(int index=0;index<xnbin;index++) { 
    ...
    if(aBarChart==TRUE)
      BarChart(aStyle.barOffset.getValue(),aStyle.barWidth.getValue(),xx,xe);
...
void SoPlotter::repBins1D_xy_curve_one(
 SoSeparator* aParent
,SoStyle& aStyle
,const std::vector<SbRepBin1D>& aBins
,const SbRepBox& aBoxX
...
void SoPlotter::repBins1D_xy_lines_one(
 SoSeparator* aParent
,SoStyle& aStyle
,const std::vector<SbRepBin1D>& aBins
,const SbRepBox& aBoxX
  ...
  if(icoord<=0) {
    ...
    if(smark==SbMarkerDot) {
      ...
      markerSet->markerIndex.setValue
        (HEPVis_SoMarkerSet::getMarkerIndex(smark,wmark));
...
,const SbString& aID
  ...
  for (unsigned int i=0;i<aContour.GetNPlanes();i++)  {
    CLineStripList* pStripList = aContour.GetLines(i);
    if(!pStripList) {
      SoDebugError::postInfo
    ...
    float val = (float)aContour.GetPlane(i);
    float zz = val;
    ...
    for (CLineStripList::iterator pos=pStripList->begin(); 
      ...
      //double xb=aContour.GetXi(index);
      //double yb=aContour.GetYi(index);
      //// retreiving index
      ////glColor4f(0,0,1,.8f);
      //index=pStrip->back();
      //double xe=aContour.GetXi(index);
      //double ye=aContour.GetYi(index);

      SbVec3f* points = new SbVec3f[pStrip->size()];
      int pointi = 0;
      int32_t* coordIndex = new int32_t[pStrip->size()+1];
      for (CLineStrip::iterator pos2=pStrip->begin(); 
           pos2 != pStrip->end() ; pos2++) {
        // retreiving index
        unsigned int index=(*pos2);
        float xx = (float)aContour.GetXi(index);
        float yy = (float)aContour.GetYi(index);
        xx = VerifyLog(xx,xmin,dx,xlog);
        yy = VerifyLog(yy,ymin,dy,ylog);
        if( 
            (xx<0) || (xx>1) ||
...
,const SbString& aID
  ...
  for (unsigned int i=0;i<aContour.GetNPlanes();i++)  {
    ...
    if(!pStripList) {
      SoDebugError::postInfo
      ("SoPlotter::repContour_xy_filled","problem with contour.");
      separator->unref();
      return;
    }
    ...
    std::vector<SbTessTriangle> tris = 
...
void SoPlotter::repErrorsI_xy(
 SoSeparator* aParent
,SoStyle& aStyle
,const std::vector<SbRepBin1D>& aBins
,const SbRepBox& aBoxX
,const SbRepBox& aBoxY
,const std::vector<float>& aBars
,float aZ
,const SbString& aID
){
  SoSceneGraph* separator = new SoSceneGraph();
...
void SoPlotter::repErrorsPlus_xy(
 SoSeparator* aParent
,SoStyle& aStyle
,const std::vector<SbRepBin1D>& aBins
,const SbRepBox& aBoxX
  ...
  for(int index=0;index<number;index++) {
    ...
    if((xx>=0)&&(xx<=1)   &&
     ...
     {int npt = 3;
      ...
      if(indexedLineSet) {
        indexedLineSet->coordIndex.setValues
          (i_coord,coordIndexn,coordIndex);
      }
      i_coord += coordIndexn;
      delete [] coordIndex;

      SbPlane plane1(SbVec3f(xx,ye,val4),
                     SbVec3f(xx,yy,val1),
                     SbVec3f(xe,yy,val2));
  ...
  for(int index=0;index<number;index++) {
    ...
    if((xx>=0)&&(xx<=1)   &&
     ...
     {int npt = 3;
      ...
      indexedLineSet->coordIndex.setValues(i_coord,coordIndexn,coordIndex);
      i_coord += coordIndexn;
      delete [] coordIndex;}
...
float bins2DToFunc(const SbPlottableBins2D& aBins,float aX,float aY){
