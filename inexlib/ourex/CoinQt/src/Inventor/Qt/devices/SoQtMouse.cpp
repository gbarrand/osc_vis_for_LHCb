...
#else
#define MetaButton MetaModifier
#define ShiftButton ShiftModifier
#define ControlButton ControlModifier
#define AltButton AltModifier
#endif
...
// ctrl-click, we have to check for "meta" on Qt/Mac >= 3.1.x
      ...
      if (mouseevent->state() & Qt::MetaButton) 
        PRIVATE(this)->buttonevent->setButton(SoMouseButtonEvent::BUTTON2);
#else
      if (mouseevent->modifiers() & Qt::MetaButton) 
        PRIVATE(this)->buttonevent->setButton(SoMouseButtonEvent::BUTTON2);
#endif
#endif 
      break;
...
#else
      conv->setShiftDown(mouseevent->modifiers() & Qt::ShiftButton);
      conv->setCtrlDown(mouseevent->modifiers() & Qt::ControlButton);
      conv->setAltDown(mouseevent->modifiers() & Qt::AltButton);
#endif
...
#else
      conv->setShiftDown(wheelevent->modifiers() & Qt::ShiftButton);
      conv->setCtrlDown(wheelevent->modifiers() & Qt::ControlButton);
      conv->setAltDown(wheelevent->modifiers() & Qt::AltButton);
#endif
