...
//   with the same defines set up by <QtCore/qglobal.h> //G.Barrand QtCore from Qt. The types
//   are INT8 and INT32.
//
// By setting up the QT_CLEAN_NAMESPACE define before including any
// Qt-headers, we make sure that <QtCore/qglobal.h> //G.Barrand QtCore won't try to make those
...
#include <QtGui/qcolormap.h> //G.Barrand QtGui
#else
...
SoQtGLWidget::getOverlayTransparentPixel(void)
{
  ...
  if (ctx) {
    ...
    return color.pixel();
#else
    QColormap cmap = QColormap::instance((-1));
...
SoQtGLWidgetP::eventFilter(QObject * obj, QEvent * e)
{
   ...
   SbString w = obj->name();
#else
    SbString w = obj->objectName().toStdString().c_str(); //G.Barrand_Qt4
#endif
    ...
    SoDebugError::postInfo("SoQtGLWidgetP::eventFilter",
                           ...
                           obj->className(),
#else
                           obj->metaObject()->className(), //G.Barrand_Qt4
#endif
                           istoplevel ? "TOPLEVEL" : "",
...
SoQtGLWidgetP::buildGLWidget(void)
{
  ...
  display = QX11Info::display(); //G.Barrand
  screen = (void*) ((unsigned long) QX11Info::appScreen()); //G.Barrand
#endif // Q_WS_X11
