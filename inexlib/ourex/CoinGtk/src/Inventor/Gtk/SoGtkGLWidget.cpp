  ...
  GtkWidget * parent)
{
  ...
  gchar* disp = gdk_get_display();
  SbName displayname(disp?disp:"unknown");
  ...
  //G.Barrand : Linux : gtk2 : the below induces a X error BadMatch.
  ...
  //FIXME
  //G.Barrand : gtk2 : the below avoid a crash in case doing a
  //                   gtk_widget_destroy on a container having a GtkGLArea.
  gdk_gl_context_ref
    (GTK_GL_AREA(PRIVATE(this)->glWidget)->glcontext); //G.Barrand
...
SoGtkGLWidget::glUnlockNormal(void)
{
  //if (GTK_IS_GL_AREA(PRIVATE(this)->glWidget))
  //  gtk_gl_area_make_current(GTK_GL_AREA(PRIVATE(this)->glWidget));
  //glXMakeCurrent(SoXt::getDisplay(), None,NULL); //G.Barrand
} // glUnlockNormal()
