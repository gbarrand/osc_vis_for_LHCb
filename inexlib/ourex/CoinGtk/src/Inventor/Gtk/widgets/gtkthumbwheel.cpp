  ...
  gint vertical)
{
  ...
  //G.Barrand : begin
  // Enforce a size here. Else problem if starting with no decoration
  // and latter set decorations.
  if (thumbwheel->vertical) {
    GtkAllocation alloc;
    alloc.x = 0;
    alloc.y = 0;
    alloc.width = GTK_THUMBWHEEL_DEFAULT_NARROW;
    alloc.height = GTK_THUMBWHEEL_DEFAULT_LENGTH;
    gtk_widget_size_allocate(GTK_WIDGET (thumbwheel),&alloc);
  } else {
    GtkAllocation alloc;
    alloc.x = 0;
    alloc.y = 0;
    alloc.width = GTK_THUMBWHEEL_DEFAULT_LENGTH;
    alloc.height = GTK_THUMBWHEEL_DEFAULT_NARROW;
    gtk_widget_size_allocate(GTK_WIDGET (thumbwheel),&alloc);
  }
  //G.Barrand : end
  ...
  GdkEventMotion * event)
{
  ...
  if (thumbwheel->state == THUMBWHEEL_DRAGGING)
  {
    ...
    thumbwheel->tempvalue =
	  ...
	  int(position - thumbwheel->downpos)); //GB : cast to int to rm warns.
