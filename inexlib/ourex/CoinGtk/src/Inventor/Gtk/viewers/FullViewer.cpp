  ...
  SbBool enable)
{
 ...
 {GtkWidget* button = PRIVATE(this)->viewbutton;
  if(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button))!=enable) {
    //if not "blocked", the set_active will emit the toggled signal.
   ...
   {GtkWidget* button = PRIVATE(this)->interactbutton;
    //if not "blocked", the set_active will emit the toggled signal.
    gtk_signal_handler_block_by_data(GTK_OBJECT(button), (gpointer) this);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),enable?FALSE:TRUE);
    gtk_signal_handler_unblock_by_data(GTK_OBJECT(button), (gpointer) this);
    }
  }}
  //G.Barrand : end
