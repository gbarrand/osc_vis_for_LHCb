...
public class OnXLab_exa_Java_SLAC_createAndFillTupleWithComplexStructure {
  ...
  public static void main(String[] argv) throws java.io.IOException {
    ...
    //FIXME : SLAC : ITree tree = treeFactory.create("testTupleWithComplexStructure.aida","type=xml;compress=no");
    ITree tree = treeFactory.create("testTupleWithComplexStructure.aida","xml",false,false,"compress=no"); //FIXME
    if(tree==null) return; //FIXME
    ...
    tuple.start();
    while ( tuple.next() ) {
      //FIXME : SLAC : ITuple momTuple = (ITuple) tuple.getObject(3);
      ITuple momTuple = tuple.getObject(3).cast_ITuple(); //FIXME
      momTuple.start();
