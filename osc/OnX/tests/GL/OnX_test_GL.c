...
static void draw_1(Window a_win){
  int    width,height;
  GLint  viewport[4];
  double aspect;

  if(glXMakeCurrent(Class.display,a_win,Class.ctx)==False)
    {
      CWarn("glXMakeCurrent failed.\n");
      return;
    }
  GetWindowSize  (Class.display,a_win,&width,&height);

 {const char* sv = (const char *)glGetString(GL_VERSION);
...
/*glFrustum      (-aspect,aspect,-1.,1.,1.,4.);*/
  ...
  glFinish       ();
  glXSwapBuffers (Class.display,a_win);

  glXMakeCurrent (Class.display,None,NULL);
}
...
/*.........................................................................*/
  ...
  win0 = CreateSimpleWindow("win 0",500    ,  500,400,200);
      ...
      if(XPending(Class.display)!=0)
        {
          ...
          if(xevent.type==Expose) {
            ...
            if(xevent.xbutton.window==win0) {

              XUnmapWindow(Class.display,win1);
              XMapWindow(Class.display,win1);
...
*/
              ...
              UniconifyWindow   (Class.display,xevent.xbutton.window);
          }
        }
    }
}
