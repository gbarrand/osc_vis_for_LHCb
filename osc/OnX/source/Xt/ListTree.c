...
#include <Xm/DrawP.h> /*G.Barrand : clang*/
...
#include <stdlib.h>
#include <stdarg.h>
void DBG(int line,const char *fcn,char *fmt, ...)
{
  va_list ap;
...
static XtResource resources[] =
{
  ...
  /*G.Barrand*/
  {XtNdoSingleClickCallback, XtCBoolean, XtRBoolean, sizeof(Boolean),
   offset(DoSingleClickCallback), XtRImmediate, (XtPointer) False}
};
...
static void RemoveReference_G_Barrand(ListTreeWidget w,ListTreeItem* item);
...
InitializeGC(ListTreeWidget w)
{
  ...
  /*G.Barrand w->list.drawGC = XtGetGC((Widget) w, mask, &values); */
  w->list.drawGC = XCreateGC(xdisp,XDefaultRootWindow(xdisp),mask, &values);

  values.function = GXinvert;
  mask = GCLineStyle | GCLineWidth | GCFillStyle | GCForeground | GCBackground | GCFont | GCFunction;
  /*G.Barrand w->list.eorGC = XtGetGC((Widget) w, mask, &values); */
  w->list.eorGC = XCreateGC(xdisp,XDefaultRootWindow(xdisp),mask, &values);

  values.background = w->list.foreground_pixel;
  values.foreground = w->core.background_pixel;
  mask = GCLineStyle | GCLineWidth | GCFillStyle | GCForeground | GCBackground | GCFont;
  /*G.Barrand w->list.highlightGC = XtGetGC((Widget) w, mask, &values); */
  w->list.highlightGC = XCreateGC(xdisp,XDefaultRootWindow(xdisp),mask, &values);
}
...
Destroy(ListTreeWidget w)
{
  ...
  /*G.Barrand XtReleaseGC((Widget) w, w->list.drawGC); */
  /*G.Barrand XtReleaseGC((Widget) w, w->list.highlightGC); */
  XFreeGC          (xdisp,w->list.drawGC);
  XFreeGC          (xdisp,w->list.eorGC);
  XFreeGC          (xdisp,w->list.highlightGC);
  ...
  while (item) {
    ...
    XtFree((char *) item);
    RemoveReference_G_Barrand(w,item);  /* G.Barrand */
    item = sibling;
  }
...
MakeActivateCallbackStruct(ListTreeWidget w, ListTreeItem * item, ListTreeActivateStruct * ret)
{
  ...
  count = 1;
  parent = item;
  while (parent && parent->parent) { /*G.Barrand*/
...
/*       ListTreeSetHighlighted(w,ret2.items,ret2.count,True); */
    ...
    if(w->list.timer_item) { /* G.Barrand */
      /* item could have been deleted by the callback. */
      /*G.Barrand : do not highlight when opening/closing a folder
      w->list.timer_item->highlighted = True; */
      w->list.timer_item->highlighted = False; /*G.Barrand*/
    }
    w->list.recount = True;
    DrawChanged(w);
  }
  ...
  if (!w->list.timer_item) {
    ...
    if (w->list.timer_id) {
      ...
      w->list.timer_id = XtAppAddTimeOut(
        ...
        (unsigned long) (w->list.DoSingleClickCallback ? 
			 1 : w->list.multi_click_time),
        SelectSingle,
        (XtPointer) w);
...
notify(Widget aw, XEvent *event, String *params, Cardinal *num_params)
{
  ListTreeWidget w = (ListTreeWidget) aw;
  if (w->list.timer_id) {
    /* don't call highlightDoCallback if we are waiting for a double click */
  }
  else if (w->list.timer_type != TIMER_CLEAR && !w->list.DoIncrementalHighlightCallback) {

    if (w->list.timer_item && w->list.timer_x < w->list.timer_item->x) {
      /*G.Barrand : HighlightDoCallback not done on folder open/close */ 
    } else {
      /*G.Barrand : in some case in which we reset the items
                    in a "open", we pass here
                    on Btn1Up and then executes also
                    "selectBranch" ! (tree_dynamic.onx).
                    This case correspond to a w->list.timer_item = 0.
                    Then we do not execute HighlightDoCallback in this case. */
      if (w->list.timer_item) /*G.Barrand*/
        HighlightDoCallback(w);
    }
...
{
  int xroot,yroot, pos;
  if(!item) return; /*G.Barrand*/
  pos=item->count;
  while (item && item->parent) { /*G.Barrand : test item*/
...
/* cut by the region */
  ...
  while (item && item->parent) item=item->parent;  /*G.Barrand:test item*/
  if (!item) return; /*G.Barrand*/
...
static void
RemoveReference_G_Barrand(w, item)
ListTreeWidget w;
ListTreeItem *item;
{
  ...
  if (item == w->list.highlighted) {
    w->list.highlighted = NULL;
  }
  if (item == w->list.drop_highlighted) {
    w->list.drop_highlighted = NULL;
  }
  if (item == w->list.timer_item) {
    w->list.timer_item = NULL;
    if (w->list.timer_id) {
      XtRemoveTimeOut(w->list.timer_id);
      w->list.timer_id = (XtIntervalId)0;
...
ListTreeItem *item;
{
  ...
  if (parent) {
    if (parent->firstchild) {
      i = parent->firstchild;
      while (i && i->nextsibling) { /*G.Barrand : test i*/
	i = i->nextsibling;
      }
      i->nextsibling = item;
      item->prevsibling = i;
    ...
    if (w->list.first) {
      i = w->list.first;
      while (i && i->nextsibling) { /*G.Barran : test i*/
	i = i->nextsibling;
      }
      i->nextsibling = item;
      item->prevsibling = i;
...
/* point to the first, even though the first was reparented. */
  while (item && item->nextsibling) { /*G.Barrand : test item*/
    item->parent = parent;
    item = item->nextsibling;
  }
...
/* Get first child in list; */
  while (item && item->prevsibling) { /*G.Barrand : test item.*/
...
/* Count the children */
  ...
  list[0] = first;
  count = 1;
  while (first && first->nextsibling) { /*G.Barrand : test first*/
...
ListTreeItem *
ListTreeTopItem(w)   /*G.Barrand*/
ListTreeWidget w;
{
  return w->list.topItem;
}

void
ListTreeSetTopItem(w,item)   /*G.Barrand*/
ListTreeWidget w;
ListTreeItem * item;
{
  w->list.topItem = item;
...
/*G.Barrand */
void ListTreeOpenCloseItem(ListTreeWidget w,ListTreeItem* item,Boolean value){
  item->open = value;
  w->list.recount = True;
}
