...
// Used in : OnX/examples/OnX/tree_dynamic.onx
...
static inlib::xml::tree* ItemML_find_itemFromPath(
...
,unsigned int& aIndex
  ...
  while(inlib::xml::tree* _tree = _for.next_tree()) {
    inlib::xml::tree* item = ItemML_find_itemFromPath(*_tree,aPath,aIndex);
    if(item) return item;
  }
  return 0;
}

static bool ItemML_visitToXML(inlib::xml::tree& aItemML,std::string& aOut){
  std::string s;
  ...
  inlib::xml::looper _for(aItemML);
  while(inlib::xml::tree* _tree = _for.next_tree()) {
    if(!ItemML_visitToXML(*_tree,aOut)) return false;
  }
...
void OnXExas_tree_initialize(Slash::UI::IUI& aUI) {
  /*
  aUI.setCallback("tree_widget","open","DLD",
		  "OnXTests OnXExas_tree_open_branch");
  aUI.setCallback("tree_widget","close","DLD",
		  "OnXTests OnXExas_tree_close_branch");
  */
  ...
  out += "  </treeItem>";
  out += "</tree>";
  aUI.setParameter("tree_widget.items",out);

}

void OnXExas_tree_open_branch(Slash::UI::IUI& aUI) {
  Slash::Core::ISession& session = aUI.session();

  std::string branch = aUI.callbackValue();

  std::ostream& out = session.cout();
  out << "debug : OnXExas_tree_open_branch :"
      << " branch " << inlib::sout(branch)
      << std::endl;
  ...
  inlib::xml::tree* branchML = ItemML_find_itemFromPath(*top,items,pathi);
  if(!branchML) return;
  ...
  if(!ItemML_visitToXML(*top,sout)) return;
  sout += "</tree>";

  aUI.setParameter("tree_widget.items",sout);
}

void OnXExas_tree_close_branch(Slash::UI::IUI& aUI) {
  Slash::Core::ISession& session = aUI.session();

  std::string branch = aUI.callbackValue();

  std::ostream& out = session.cout();
  out << "debug : OnXExas_tree_close_branch :"
      << " branch " << inlib::sout(branch)
      << std::endl;
...
void OnXExas_tree_select(Slash::UI::IUI& aUI) {
  Slash::Core::ISession& session = aUI.session();

  std::string s = aUI.callbackValue();

  std::ostream& out = session.cout();
  out << "debug : OnXExas_tree_select :"
      << " " << inlib::sout(s) 
      << std::endl;
}
