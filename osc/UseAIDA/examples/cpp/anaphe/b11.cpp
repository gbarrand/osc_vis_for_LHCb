...
int main( int, char** )
{
  ...
  if(!tree.get()) return EXIT_FAILURE;
  ...
  std::auto_ptr<AIDA::IDataPointSet> dpsa( dpsf->create( "my_dps", "Example of an x-y data point set", 2) );
  if(!dpsa.get()) return EXIT_FAILURE;
