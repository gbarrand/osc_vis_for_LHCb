...
FOR /F "usebackq delims=;" %%s IN (`slash-config --incs`) DO SET slash_incs=%%s
FOR /F "usebackq delims=;" %%s IN (`osc-config --iv_cppflags`) DO SET iv_incs=%%s

SET flags=/nologo /DWIN32 /MD /O2 /W3  /GX /GR 

IF EXIST %EXA%.obj DEL %EXA%.obj
CL.exe %flags% /c %aida_incs% %amanip_incs% %slash_incs% %iv_incs% /I.\lib /Fo%EXA%.obj /Tp%EXA%.cpp 1> cl_exe_error
