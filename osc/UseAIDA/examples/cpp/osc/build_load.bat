...
CL.exe %flags% /c /I.\lib /FoLoader.obj /Tp.\lib\Loader.cpp 1> cl_exe_error
IF ERRORLEVEL 1 TYPE cl_exe_error && GOTO obuild_return
IF NOT ERRORLEVEL 1 DEL cl_exe_error 

IF EXIST %EXA%.obj DEL %EXA%.obj
CL.exe %flags% /c %aida_incs% %inlib_incs% /I.\lib /Fo%EXA%.obj /Tp%EXA%.cpp 1> cl_exe_error
IF ERRORLEVEL 1 TYPE cl_exe_error && GOTO obuild_return
IF NOT ERRORLEVEL 1 DEL cl_exe_error 

LINK.exe /nologo /OPT:NOREF /out:AIDA_example_%EXA%.exe %EXA%.obj Loader.obj %aida_libs% 1> link_exe_error
