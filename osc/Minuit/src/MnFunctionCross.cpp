...
//       double zlim = 0.;
  ...
  if(!min0.isValid()) return MnCross(nfcn);
  ...
  if(!min1.isValid()) return MnCross(nfcn);
...
//       if(dfda > 0.) goto L460;
    ...
    if(ipt > maxitr) return MnCross(nfcn);
  } //if(dfda < 0.)
...
L460:
  ...
  if(noless == 0 && ibest != 2) return MnCross(nfcn);
  if(noless == 3 && ibest != 2) {
    alsb[1] = alsb[2];
    flsb[1] = flsb[2];
    goto L300;
...
L500:
  ...
  do {
    ...
    if(determ < prec.eps()) return MnCross(nfcn); 
