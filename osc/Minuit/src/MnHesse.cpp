...
L26:  
	...
	vhmat(j,j) = tmp < prec.eps2() ? 1. : tmp;
      ...
      return MinimumState(st.parameters(), MinimumError(vhmat, MinimumError::MnHesseFailed()), st.gradient(), st.edm(), st.nfcn()+mfcn.numOfCalls());
...
L30:      
  ...
  if(ifail != 0) {
    ...
    return MinimumState(st.parameters(), MinimumError(tmpsym, MinimumError::MnHesseFailed()), st.gradient(), st.edm(), st.nfcn()+mfcn.numOfCalls());
  }
